...
*/
 ...
 LEFT JOIN users ON vendorsinvoices.RaisedBy = users.uid
  ...
  WHERE vendorsinvoices.Status= 1 AND vendorsinvoices.isactive= 1  ORDER BY vendorsinvoices.cid
...
//fetch tha data from the database
	 if ($NoRowLI > 0) 
   {
    ...
    if($FileLink != ""){
    $FileLinkn = '<a target="_blank" title="Download Invoice" href="'.$FileLink.'"><i class="fa fa-link"></i></a>';
    }
    else
    {
      $FileLinkn = '';
