...
$resultRFQ1 = mysql_query("SELECT DISTINCT RequestID, Approved, Deparment FROM cashreq 
  LEFT JOIN department ON cashreq.Deparment = department.id 
    LEFT JOIN divisions ON department.DivID = divisions.divid 
    WHERE (divisions.GM='".$myGMID."'  AND cashreq.Approved=3 AND cashreq.isActive=1) OR (divisions.GM='".$myGMID."'  AND cashreq.Approved=4 AND cashreq.isActive=1) OR (cashreq.Approved=6 AND cashreq.isActive=1)");
    $NoRowRFQ1 = mysql_num_rows($resultRFQ1); 
