...
$resultDept = mysql_query("SELECT * FROM department");
...
$resultDiv = mysql_query("SELECT * FROM divisions WHERE divid =1");
...
divisions.DivisionName As DivNme, divisions.DH, divisions.GM  FROM poreq 
   LEFT JOIN department ON poreq.Deparment = department.id 
    LEFT JOIN divisions ON department.DivID = divisions.divid
    WHERE ( ((poreq.Approved=3) OR (poreq.Approved=4 ) OR poreq.Approved=6) AND poreq.isActive=1)");
...
if ($sReqID != "") 
{
  ...
  LEFT JOIN department ON poreq.Deparment = department.id 
     WHERE RequestID='$sReqID' AND poreq.isActive=1");
  //LEFT JOIN divisions ON department.DivID = divisions.divid
} 
else 
{
...
function printDiv(divName) {
            ...
            <div class="col-md-12">
              <div class="box">
                <div class="box-header with-border">
                  <h3 class="box-title">GM <strong>[<?php echo $DivisionName; ?>]</strong> Pending Approval</h3>
   ...
   <div class="form-group has-feedback" style="width:90%; display: inline-block; margin:12px; ">
    ...
    <div class="form-group has-feedback">
      <div>
        <?php if($sReqID != "" AND $ApprovedChk == 4) {  ?>
          <?php if($GMDD == $HODID) { ?>
       <button class="btn btn-success pull-right" onclick="sendTOMD()" type="button"> Send to MD | <i class="fa fa-send"></i></button>
     <?php } else { ?>
     ...
     <?php } ?>
        ...
        <?php if($CPApp != "") { ?>
         <button class="btn btn-warning pull-left" onclick="sendBTOCnP()" type="button"> Send back to Contracts & Procurements | <i class="fa fa-send"></i></button>
       <?php } else { ?>
         <button class="btn btn-info pull-left" onclick="sendBTOHODiv()" type="button"> Send back to Head of Division | <i class="fa fa-send"></i></button>
     <?php } 
      } ?>
      </div>
    </div>
     ...
     function sendBTOCnP()
      {
        ...
        //var OptType =   loadAccType(acctclass); //'<?php echo $OptType ?>';
          ...
          var size='standart';
                  var content = '<form role="form" action="sendTOBCnP" method="POST" ><div class="form-group">' +
                  ...
                  var title = 'Send Back To Contracts & Procurements';
          ...
          var size='standart';
                  var content = '<form role="form" action="sendTOBDivHead" method="POST" ><div class="form-group">' +
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>PDF CODE: </label><input type="text" class="form-control"  name="ReqCode" value="'+ sReqID +'"  readonly ></div>' +
                    ...
                    '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Message to Head of Division: </label><input type="text" class="form-control" id="eaccCode" name="ReqMSG" placeholder=""  value="Kindly update and submit again. Thanks" required ></div>' +
      ...
      function sendTOGMDD()
      {
        ...
        var DivisonName = '<?php echo $DivisonName ?>';
          ...
          var size='standart';
                  var content = '<form role="form" action="sendTOGMDD" method="POST" >'+
    ...
    <script type="text/javascript">
       ...
       function sendTOMD()
      {
        ...
        var DivisonName = '<?php echo $DivisonName ?>';
          ...
          var size='standart';
                  var content = '<form role="form" action="sendTOMDcs" method="POST" >'+
                  '<div class="form-group">' +
                   '<input type="hidden" value="" id="accID" name="accID" />'+
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>PDF CODE: </label><input type="text" class="form-control"  name="ReqCode" value="'+ sReqID +'"  readonly ></div>' +
...
function ReadLineItem(elem)
    {
     ...
     if (hhh != "")
     {     
    window.location.href ="mgcsppor?pdfid=" + hhh;
    //window.alert("JKJ");
     }
