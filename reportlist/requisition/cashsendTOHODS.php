...
if($isHOD < 1) { 
	...
	header('Location: cashsupervisorppor?pdfid='.$ReqCode);
	exit; 
}
while ($row = mysql_fetch_array($sql_res)) {
      $REQUID = $HODID = $row['uid'];
       $HODFullname = $row['Firstname'] . " " . $row['Surname'];
       $HODEmail = $row['Email'];
     }

//Update the rpo table
   mysql_query("UPDATE cashreq SET Approved='2', LastActor='Supervisor of Department', Status='Approved', DeptHeadApp='".$HODID."', SupervisorApp='".$uid."', ApprovedBy='".$_SESSION['Firstname']. " ".$_SESSION['SurName']."', SupervisorAppDate='".$TodayD."', SupervisorComment='".$hodMSG."' WHERE RequestID = '$ReqCode'");
