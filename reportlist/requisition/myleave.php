...
if ($NoRow > 0) 
{
	...
	while ($row = mysql_fetch_array($resultLeave)) {
     ...
     $Approved = $row['Status'];
      if ($Approved == 0) {
        $Approved = '<a href="sendforapproval?id='.$reqid.'"><button title="submit leave request" class="btn btn-success"><i class="fa fa-send"></i> Submit</button></a> &nbsp; <a href="del?id='.$reqid.'"><button title="delete leave request" class="btn btn-danger"><i class="fa fa-trash"></i></button></a>';
      }
      else if ($Approved == 1)
      {
