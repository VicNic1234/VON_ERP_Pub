...
$RecKPINme = "";
$resultChartType = mysql_query("SELECT * FROM ppe_kit");
...
if ($NoRowChartType > 0) {
  while ($row = mysql_fetch_array($resultChartType)) {
    ...
    $RecKPINme .= '<tr><td>'.$id.'</td><td>'.$ppeNme.'</td>
    
    <td><a ppeNme="'.$ppeNme.'" ppeid="'.$id.'" onclick="editAcc(this)" ><i class="fa fa-edit"></i></a></td>
    </tr>';
    }
...
{
 var ppeNme = $(elem).attr("ppeNme"); 
 var ppeid = $(elem).attr("ppeid");
 //alert(ppeid); 
...
*/
    var size='standart';
            var content = '<form role="form" action="editPPE" method="POST" >' +
             '<input type="hidden" value="'+ppeid+'" id="ppeid" name="ppeid" />'+
            
              '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>PPE Name: </label><input type="text" class="form-control" id="PPENme" name="PPENme" value="'+ppeNme+'" required ></div>' +
             '<button type="submit" class="btn btn-primary pull-right">Update</button><br/></form>';
            var title = 'Edit PPE Kit';
           ...
           <div class="col-md-12">
              <div class="box">
                <div class="box-header with-border">
                  <h3 class="box-title">PPE Kits</h3>
                  <div class="box-tools pull-right">
                  </div>
                </div><!-- /.box-header -->
              <div class="box">
                <div class="box-header">
                 <button class="btn btn-success pull-right" id="addAcct" onclick="addAcc(this)"> Add PPE Kit</button>
