...
$CONID = $_GET['cnid'];
 $resultLI = mysql_query("SELECT *, cuspo.Comment AS RFQComment FROM cuspo 
 LEFT JOIN customers ON cuspo.cusid = customers.cusid
 LEFT JOIN users ON cuspo.PEAID = users.uid
 LEFT JOIN businessunit ON cuspo.RFQBusUnit = businessunit.id
 WHERE CUSPOid='".$CONID."'
...
if ($NoRowContracts > 0) {
  while ($row = mysql_fetch_array($resultContracts)) 
  {
    ...
    if($CorContract == $Concid)
    {
     $ContractOpt .= '<option value="'.$Concid.'" selected >'.$ConTNO.'</option>';
    }
    else
    {
     $ContractOpt .= '<option value="'.$Concid.'">'.$ConTNO.'</option>';
    }
...
if ($NoRowTerm > 0) {
  while ($row = mysql_fetch_array($resultTerm)) 
  {
    ...
    $Terms .= '<tr>
      ...
      /*Get Line Items*/
$resultSRFQ = mysql_query("SELECT * FROM lineitems Where RFQCode='".$CONID."'");
...
?>
            ...
            <div class="col-md-12">
              <div class="box">
                <div class="box-header with-border">
                  <h3 class="box-title">Customer's PO Details</h3>
          ...
          <div class="form-group has-feedback col-md-12" >
                ...
                <!--<select class="form-control srcselect" id="CusSource" name="CusSource" required>
                <option value=""> Choose Customer</option>
                <?php echo $CusOpt; ?>
                </select>-->
                <input type="text" name="" class="form-control" value="<?php echo $CustormerNme; ?>" readonly > 
                 <input type="hidden" name="CUSPOIDn" value="<?php echo $CONID; ?>">
     ...
     <div class="form-group has-feedback col-md-12">
       <textarea rows="4" cols="50" placeholder=" Enter comment here..." id="Comment" name="Comment" style="width:100%; display: inline-block;"><?php echo $Comment; ?></textarea>
          ...
          <div class="form-group has-feedback">
            <input type="text" class="form-control" value="<?php echo $PONo; ?>" id="PONo" name="PONo" placeholder=""  required />
...
<br/>
      ...
      <div class="form-group has-feedback">
        ...
        <br/>
          <?php if($FileLink != "") { ?>
                    <center><strong><a href="<?php echo $FileLink; ?>"> Cus. PO Master Document </a></strong></center>
                    <?php } ?>
                    <br/>
      <p class="text-center">
                        <strong>PO Document to Overwrite Previous (Optional)</strong>
                      </p>
      <div class="form-group has-feedback">
            <input type="file" id="POFile" name="POFile" class="form-control" />
          ...
          <div class="row">
            ...
            <div class="col-xs-12">
              <button type="submit" class="btn btn-success btn-block btn-flat">Update Cus. PO</button>
            </div><!-- /.col -->
          </div>
                      </form> 
                    </div><!-- /.col -->
