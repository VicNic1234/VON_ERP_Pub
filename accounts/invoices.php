...
$SelectedYear = strip_tags($_GET['SelectedYear']);
//exit;
require '../DBcon/db_config.php';

  $PageTitle = "Vendor Invoice Lists";
 $InvoiceYears .= '<option value="ALL"> ALL </option>';  
 
$resultIVList = mysql_query("SELECT DISTINCT  SUBSTRING(IVDate, 1, 4) AS ExtractString FROM acct_vendorsinvoices");
        ...
        while ($row = mysql_fetch_array($resultIVList)) 
                {
                  if($row['ExtractString'] != "")
                  {
                    if($PresentBusYear == $row['ExtractString'] && $SelectedYear == "") {  
                           $InvoiceYears .= '<option value="'.$row['ExtractString'].'" selected > '.$row['ExtractString'].' </option>'; 
                    }
                    else if($SelectedYear != "" && $SelectedYear == $row['ExtractString']) {  
                           $InvoiceYears .= '<option value="'.$row['ExtractString'].'" selected > '.$row['ExtractString'].' </option>'; 
                    }
                    
                    else { $InvoiceYears .= '<option value="'.$row['ExtractString'].'" > '.$row['ExtractString'].' </option>'; }
                  }
                ...
                <div class="col-md-6 col-md-offset-3" style="background-color: #FF6868; border-radius: 25px;">
                    <form role="form" action="" method="GET" ><div class="form-group">
                      <div class="form-group col-md-6">
                        ...
                        -->
                        <select class="form-control" name="SelectedYear" > <?php echo $InvoiceYears; ?> </select>
                      </div>
                      <!--<div class="form-group col-md-6">
                        <label>To: </label>
...
{
    $ExtSQL = "AND IVDate like '%".$SelectedYear."%'";
}
else if ($SelectedYear != "ALL")
{
    $ExtSQL = "AND IVDate like '%".$PresentBusYear."%'";
}
else
{
    $ExtSQL = "";
 ...
 LEFT JOIN users ON acct_vendorsinvoices.RaisedBy = users.uid
  WHERE acct_vendorsinvoices.isActive=1 ".$ExtSQL."  ORDER BY acct_vendorsinvoices.cid
 "); //WHERE isActive=1 ORDER BY cid //GROUP BY PONUM
...
//fetch tha data from the database
	 if ($NoRowLI > 0) 
   {
    ...
    if($SETR == 1)
    {
    $AccessState = '<input type="checkbox" checked id=sr"'.$cid.'" sr="'.$cid.'" onclick="setR(this)" title="tick to allow GM CS have access to this Invoice" >';
    }
    else
    {
       $AccessState = '<input type="checkbox" id=sr"'.$cid.'" sr="'.$cid.'" onclick="setR(this)" title="tick to allow GM CS have access to this Invoice" >';
    }
    ...
    if($PostID == 0) { $PostID = '<b style="color:red">Not Acknowledged</b>'; }
    else{ 
        if($BAL == 0)
        {
        $PostID = '<b style="color:green">Paid</b>';
        }
        elseif ($BAL == $TotalMain)
        {
            $PostID = '<b style="color:blue">Acknowledged</b>';
        }
        else{
            $PostID = '<b style="color:blue">Partial Payment</b>';
        }
...
?>	
      ...
      function setR(elem)
      {
        var AccessStateID = $(elem).attr('sr'); 
        //alert("God, I Love You So Much");
        var SETR = $(elem).is(':checked');
        if(SETR == true) { SETR = 1; } else { SETR = 0; } 
       // alert(SETR);

        var dataString = {sr:AccessStateID, SETR:SETR};
