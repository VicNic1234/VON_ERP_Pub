...
//Let's Read Earnings
$RecAccSet = "";
$resultAccSet = mysql_query("SELECT * FROM opt_earning_freq");
$NoRowAccSet = mysql_num_rows($resultAccSet);
if ($NoRowAccSet > 0) {
  while ($row = mysql_fetch_array($resultAccSet)) {
    $id = $row{'enfreqid'};
    $DescriptionEnrs = $row['EarningFrequ'];
    $WorkHrs = $row['WorkHrs'];
    $OneOff = $row['OneOff'];
    $CreatedOn = $row['Date'];
...
function addAcc(elem)
{
    ...
    var size='standart';
            ...
            '<div class="col-md-12">' +
            '<form role="form" action="addEarnFreq" method="POST" ><div class="form-group">' +
              '<div class="form-group col-md-6"><label>Description: </label><input type="text" class="form-control" name="newEarnFreq" placeholder="Enter Earning Freq. Description"></div>' +
              '<div class="form-group col-md-6"><label>Work Hours: </label><input  type="number" class="form-control" name="newWrkHrs" /></div>' +
              '<div class="form-group col-md-6"><label>Non-Recurrent: </label><select class="form-control" name="newOneOff"><option value="YES">YES</option><option value="NO">NO</option></select></div>' +
              '<div class="form-group col-md-6"><label>Active: </label><select class="form-control" name="newNature"><option value="1">YES</option><option value="0">NO</option></select></div>' +
              '<button type="submit" class="btn btn-primary pull-right">Add Earning Frequency</button><br/></form>'+
              '</div></div>';
            var title = 'New Earning Frequency';
            var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>';
           ...
           <div class="col-md-12">
              ...
              <div class="box">
                <div class="box-header">
                  <h4> Earning Frequency </h4>
                 <button class="btn btn-success pull-right" id="addAcct" onclick="addAcc(this)" > Add Earning Frequency</button>
                </div><!-- /.box-header -->
                <div class="box-body">
