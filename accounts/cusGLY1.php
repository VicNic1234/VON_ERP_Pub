...
if ($NoRowSUP > 0) {
 ...
 //
//AND postings1.GLImpacted = 589
//AND (postings1.GLImpacted = 589 OR TransacType = 'DEBIT') //
//Let's Read ChartClass
//postings1.CusID = $CusID AND
//OR (TransacType = 'DEBIT' AND postings1.CusID = $CusID)
//(tncid =  217 OR CounterTrans = 217 AND postings1.GLImpacted <> 589)
//OR (GLDescription like '%Customer Debit Bal%' AND postings1.CusID = $CusID)
...
$TCRD = 0.0; $TDRD = 0.0; $CHKBAL = 0.0;
    $resultChartMaster = mysql_query("SELECT *, postings1.tncid As TNID, enlinvoices.cid As ivcid, reciepts.CheuqeNME As CHKNME  FROM postings1
             ...
             WHERE (postings1.GLImpacted = 589 AND TransacType = 'DEBIT' AND postings1.CusID = '$CusID' AND postings1.isActive=1 AND postings1.TransactionAmount > 0 )   
              OR
              (postings1.GLImpacted <> 589 AND TransacType = 'DEBIT' AND postings1.CusID = '$CusID' AND postings1.isActive=1 AND postings1.TransactionAmount > 0 )   
            ...
            if ($NoRowChartMaster > 0) {
              while ($row = mysql_fetch_array($resultChartMaster)) {
                ...
                //if($GLDescriptionPB == "Trade Receivable") { $TT = "CREDIT"; }
                if($acid == 589) { $TT = "DEBIT"; } else { $TT = "CREDIT"; }
