...
$TOTALHEADBALT = 0.0;
 ...
 //while ($row = mysql_fetch_array($ChartMstQ)) 
 {
          $GLIMPTY = 749; //$row['mid'];
           ...
           $mRecChartMaster = "";
          //////////////////////////////////////////////////////////////////////////
          $TOTALHEADBAL = 0.0; $TOTALHEADDR = 0.0; $TOTALHEADCR = 0.0; $TOTALHEADopenbalDR = 0.0; $TOTALHEADopenbalCR = 0.0; 
  
             $resultChartMaster = mysql_query("SELECT *, postings.ChqNo As ChqNoM, acc_chart_class.cid AS ACLASS, suppliers.isActive As SupActive FROM postings 
            ...
            LEFT JOIN suppliers ON postings.VendorID =  suppliers.supid
            ...
            if ($NoRowChartMaster > 0) {
              while ($row = mysql_fetch_array($resultChartMaster)) {
                ...
                $NowCus = $row['CustormerNme'];
                $NowVend = $row['SupNme'];
                if($NowVend != "") { $SupplierName = '<b>Vendor :</b> ' . $row['SupNme']; } else { $SupplierName = ""; }
                if($NowCus != "") { $CustormerName = '<b>Customer :</b> ' . $row['CustormerNme']; } else { $CustormerName = ""; }
            ...
            elseif ($FromD != "" && $ToD != "")
            {
                    if ($cid == 3 || $cid == 4)
                    {
                        if ( $FromD <= $TD && $TD <= $ToD) // Income And Expenses transactions
                        {
                        if($TT =="CREDIT") 
                        { 
                          $CRD = $TAmount;  
                        ...
                        { $DRD = ''; }
                            ...
                            $mRecChartMaster .= '<tr><td>-</td><td>-</td><td>'.$acid.'</td><td><a href="tbviewacct?TID='.$cidIM.'"><i class="fa fa-eye"></i></a> '.$name.'</td><td>'.$TD.'</td><td><a href="tbviewacct?ACCTID='.$mid2.'" ><i class="fa fa-eye text-green"></i></a> '.$mid3.' :: '. $GHT.'</td><td>'.$SupplierName.' '.$CustormerName.' -- '.$TDescr.'</td><td>'.number_format($DRD,2).'</td><td>'.number_format($CRD,2).'</td>
                            <td></td><td></td></tr>';
                        ...
                        if ( $FromD > $TD)// $FromD <= $TD && // Brought Forward for Income And Expenses to Move to Retained Earnings
                            {
                                if($TT =="CREDIT") 
                                { 
                                  $CRD = $TAmount;  $DRD = 0;
                                 ...
                                 // $TOTALHEADCR += floatval($TAmount);
                                   ...
                                   //$RecChartMasterGRTN .= '<tr><td>-</td><td>-</td><td>'.$acid.'</td><td><a href="viewacct?TID='.$cidIM.'"><i class="fa fa-eye"></i></a> RETAIN EARNING - '.$name.'</td><td>'.$TD.'</td><td><a href="viewacct?ACCTID='.$mid2.'" ><i class="fa fa-eye text-green"></i></a> '.$mid3.' :: '. $GHT.'</td><td>'.$SupplierName.' '.$CustormerName.' -- '.$TDescr.'</td><td class="rth-bal-dr">'.number_format(abs($DRD),2).'</td><td class="rth-bal-cr">'.number_format($CRD,2).'</td><td></td><td></td></tr>';
                                } 
                                else 
                                { $CRD = ''; }
                                ...
                                if($EXZ != 1) //DETAILS1
                                { 
                                // $RecChartMaster .= '<tr><td>-</td><td>-</td><td>'.$acid.'</td><td><a href="viewacct?TID='.$cidIM.'"><i class="fa fa-eye"></i></a> '.$name.'</td><td>'.$TD.'</td><td><a href="viewacct?ACCTID='.$mid2.'" ><i class="fa fa-eye text-green"></i></a> '.$mid3.' :: '. $GHT.'</td><td>'.$SupplierName.' '.$CustormerName.' -- '.$TDescr.'</td><td>'.number_format($DRD,2).'</td><td>'.number_format($CRD,2).'</td>
                                // <td></td><td></td></tr>';
                                }
                    ...
                    {
                       // if ( $FromD <= $TD && $TD <= $ToD)
                        if ( $FromD <= $TD && $TD <= $ToD)// $FromD <= $TD && //  Assest And Liabilities $cid
                            {
                                if($TT =="CREDIT") 
                                { 
                                  $CRD = $TAmount;  
                        ...
                        if ($FromD > $TD)// $FromD <= $TD && // Brought Forward for Assest And Liabilities $cid
                            {
                                ...
                                { 
                                  ...
                                  $SuperCredit += floatval($TAmount);
                                  $TOTALHEADCR += floatval($TAmount);
                                  $TOTALHEADopenbalCR += floatval($TAmount);
                                ...
                                { 
                                  ...
                                  $SuperDebit += floatval($TAmount);
                                  $TOTALHEADDR += floatval($TAmount);
                                  $TOTALHEADopenbalDR += floatval($TAmount);
               ...
               /////////////////////////////////////////////////
                ...
                { 
                   ...
                   if($TOTALHEADopenbalCR > 0 || $TOTALHEADopenbalDR > 0 ) {
                     $FObalCr = $TOTALHEADopenbalCR - $TOTALHEADopenbalDR;
                    if($FObalCr > 0) { $TOTALHEADopenbalCR = $FObalCr; $TOTALHEADopenbalDR = 0; } else { $TOTALHEADopenbalDR = abs($FObalCr); $TOTALHEADopenbalCR = 0; }
                    
                      $RecChartMaster .= '<tr><td>-</td><td>-</td><td>'.$acid.'</td><td><a href="tbviewacct?TID='.$cidIM.'"><i class="fa fa-eye"></i></a> '.$name.'</td><td>-</td><td>Opening Bal</td><td>Opening Bal</td><td class="rth-dr">'.number_format($TOTALHEADopenbalDR,2).'</td><td class="rth-cr">'.number_format($TOTALHEADopenbalCR,2).'</td>
                                <td></td><td>  </td></tr>';
                  
                   }
                   
                   //MAIN AFTER OPEN BAL/////////////////////////
                   $RecChartMaster .= $mRecChartMaster;
                   //////////////////////////////////
                  if($cidIM == 749) {
                   ...
                   // $RecChartMaster .= $RecChartMasterGRTN; //GRACE GRACE OF GOD
                    $RecChartMaster .= '<tr id="rth-bal-m"><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"> BAHHH </td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: double; color:green; " id="rth-bal-m-dr">'.number_format(abs($TOTALHEADBALdrn),2).'</td><td id="rth-bal-m-cr" style="border-top: double; color:green; ">'.number_format(abs($TOTALHEADBALcrn),2).'</td></tr>';
                  ...
                  {
                      $TOTALHEADBALcrn = 0; $TOTALHEADBALdrn = 0; if($TOTALHEADBAL < 0) { $TOTALHEADBALdrn = $TOTALHEADBAL; } else { $TOTALHEADBALcrn = $TOTALHEADBAL; }
                    $RecChartMaster .= '<tr><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: double; color:green; ">'.number_format(abs($TOTALHEADBALdrn),2).'</td><td style="border-top: double; color:green; ">'.number_format(abs($TOTALHEADBALcrn),2).'</td></tr>';
          ...
          ////////////////////////////////////////////////////////////////////////////////
              ...
              // </tr>';
                    $RecChartMaster .= '<tr> <td> - </td> <td>-</td> <td>-</td> <td>-</td> <td>-</td> <td>-</td> <td>-</td> <td>-</td> <td>-</td>
...
?>
                ...
                <div class="box-header with-border">
                  ...
                  <div class="row">
                    <div class="col-md-12">
                        ...
                        <form role="form" action="mglreportRT" method="POST" ><div class="form-group">
    ...
    // $('#rth-bal-m').closest( "tr" ).before(RecChartMasterGRTN);
     ...
     console.log("Generated Retained CR " + mainRetainedEarningCR); // alert(mainRetainedEarningCR);
      var mainRetainedEarningDR = '<?php echo $Generated_RetainedDR; ?>';
      console.log("Generated Retained DR " + mainRetainedEarningDR); // alert(mainRetainedEarningDR);
      var rthCR = $('#rth-bal-m-cr').html(); var rthDR = $('#rth-bal-m-dr').html();
       console.log("4MACR " + rthCR);   console.log("4MADR " + rthDR); 
      ...
      //var nFRDR = FRDR.toLocaleString('en-US', {minimumFractionDigits: 2});
       ...
       $('#rth-bal-m-cr').html(nRETAINBAL); //$('#rth-bal-m-dr').html(nFRDR);
        ...
        var GENERATEDRT = Number(mainRetainedEarningCR) - Number(mainRetainedEarningDR);
         var nGENERATEDRT = GENERATEDRT.toLocaleString('en-US', {minimumFractionDigits: 2});
       var MKUPGRTN = "<tr><td> </td> <td> </td> <td></td> <td> RETAIN EARNING </td> <td> </td> <td> </td> <td> Broght Froward of INCOME and EXPENSES </td><td> "+ 0 +"</td><td> "+ nGENERATEDRT +"</td></tr>";
        $('#rth-bal-m').closest( "tr" ).before(MKUPGRTN);
