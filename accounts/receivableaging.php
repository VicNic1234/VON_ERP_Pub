  ...
  while ($row = mysql_fetch_array($resultLI)) 
  {
    ...
    //if($amtOweing > 0)
    {
        ...
        //if($AMTPOSTED[0] > 0)
        {
         ...
         $TotalAmtPaidArry = getTotalAmtPaid($cusid);
         $FTotalAmtPaid = $TotalAmtPaid = $TotalAmtPaidArry[0];
         ...
         $PostedAmtMAC += $TFINAL; $D0T30 += $AMTPOSTED[1]; $D31T60 += $AMTPOSTED[2]; $D61T90 += $AMTPOSTED[3]; $D91UP += $AMTPOSTED[4];
        
        //<td>'.number_format($TotalAmtPaid, 2).'</td>
            if(abs($TFINAL) > 0)
            {
            $AGETAB .= '<tr>
                                <td><a target="_blank" href="mcusGLY?IJK='.$cusid.'">'.$CusName.'</a></td>
