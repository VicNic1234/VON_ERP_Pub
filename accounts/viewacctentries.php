      ...
      if ($NoRowType > 0) {
        ...
        if($TID != ""){
           /* if($TID == "ALL")
            ...
            {
          $resultChartMaster = mysql_query("SELECT *, postings.ChqNo As ChqNoM, postings.tncid As TNCID, suppliers.isActive As SupActive FROM postings
             LEFT JOIN acc_chart_master ON postings.GLImpacted = acc_chart_master.mid
             LEFT JOIN cheuqes ON postings.ChqNo = cheuqes.chid
        ...
        if($ACCTID != ""){
          $resultChartMaster = mysql_query("SELECT *, postings.ChqNo As ChqNoM, postings.tncid As TNCID FROM postings
            ...
            if ($NoRowChartMaster > 0) {
              while ($row = mysql_fetch_array($resultChartMaster)) {
               ...
               $CusIDM = getCustomer($row['VendorID']);
                ...
                $UpdBtn = '<span onclick="updateVAL('.$TNCID.')" title="Update Posting" class="btn btn-sm btn-primary"><i class="fa fa-edit"></i></span>';
                }
                if($ChqNoM != "")
                {
            ...
            if($FromD == "")
            {
                ...
                $RecChartMaster .= '<tr><td><b>'.$AccTClass.'</b></td><td><em><b>'.$tname.'</em></td><td>'.$acid.'</td><td><a href="tbviewacct?TID='.$cid.'"><i class="fa fa-eye"></i></a> '.$name.'</td><td>'.$TD.'</td><td><a href="tbviewacct?ACCTID='.$mid2.'"><i class="fa fa-eye text-green"></i> ' .$CusIDM.'</a> '.$mid3.' '.$CHPK.'</td><td>'.$SupplierName.' '.$CustormerName.' -- '.$TDescr.'</td><td>'.$ReceivedByM.'</td><td id="dr-'.$RowID.'">'.number_format($DRD,2).'</td><td id="cr-'.$RowID.'">'.number_format($CRD,2).'</td>
           ...
           <div class="col-md-12">
                  ...
                  <div class="row">
                    <div class="col-md-12">
                       ...
                       <!-- <div class="col-md-10 col-md-offset-1" style="background-color: #FF6868; border-radius: 25px;">
                        <form role="form" action="tbviewacct"  >
                          ...
                          <div class="form-group col-md-6">
                            <label>GL Acct: </label>
                            <select class="form-control select2" id="srcselect2"  name="TID" required >
                                <?php echo $GLOpt; ?>
                            </select>
                          </div>
              ...
              <div class="box">
                <div class="box-header">
                 ...
                 <button class="btn btn-sm btn-success pull-left" onclick="ExportToExcel()" > Export To Excel</button>
                 <a href="delacct?p=<?php echo $TNCID; ?>"> <button class="btn btn-sm btn-danger pull-right"  > Delete All Entries </button> </a>
         ...
         function updateVAL(tnic)
         {
            ...
            success: function(html)
            {
                  ...
                  var content = '<div class="row" style="background:#EEE6E6; border-radius:5px;" > <div class="col-md-12"><form role="form" action="updatepost" method="post" ><div class="form-group">' +
                    ...
                    '<div class="form-group col-md-12"><label>Received By: </label><select class="form-control"  name="ReceivedBy" id="ReceivedBy" disabled >'+OptStaff+'</select></div>' +

                    '<div class="form-group col-md-12"><label>Select GL Account: </label><select class="form-control select2"  name="PostGL" id="PostGL" >'+GLOpt+'</select></div>' +
                    ...
                    */
                     ...
                     '<div class="form-group col-md-6"><label>Posted Date: </label><input type="text" class="form-control" id="PostDate" value="'+TransactionDate+'" disabled  name="PostDate" required readonly ></div>' +
