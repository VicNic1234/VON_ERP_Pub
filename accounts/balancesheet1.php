...
if ($NoRowClass > 0) {
  while ($row = mysql_fetch_array($ChartClassQ)) {
    ...
    if($DisAmt != 0 && $EXZ == 1)
    {
        ...
        $RecChartMaster .= '<tr><td><a href="viewacct?TID='.$cid.'" target="_blank"><i class="fa fa-eye"></i></a> <em>'.$AccTClass.'</em></td><td>-</td><td>'. DisplayAmt($DisAmt).'</td><td>&nbsp;</td></tr>';
    }
    else if ( $EXZ == 0)
     {
...
if ($NoRowClass > 0) {
  while ($row = mysql_fetch_array($ChartClassQ)) {
   ...
   if($DisAmt != 0 && $EXZ == 1)
    {
        $T10 += floatval($DisAmt); 
        $RecChartMaster .= '<tr><td><a href="viewacct?TID='.$cid.'" target="_blank"><i class="fa fa-eye"></i></a> <em>'.$AccTClass.'</em></td><td>-</td><td>'.  ($DisAmt).'</td><td>&nbsp;</td></tr>';
    }
    else if ( $EXZ == 0)
     {
        $T10 += floatval($DisAmt);
...
if ($NoRowClass > 0) {
  while ($row = mysql_fetch_array($ChartClassQ)) {
    ...
    {
        $T4 += floatval($DisAmt);
        $RecChartMaster .= '<tr><td><a href="viewacct?TID='.$cid.'" target="_blank"><i class="fa fa-eye"></i></a> <em>'.$AccTClass.'</em></td><td>-</td><td>'.  rmvneg($DisAmt).'</td><td>&nbsp;</td></tr>';
    }
    else if ( $EXZ == 0)
     {  $T4 += floatval($DisAmt);
        $RecChartMaster .= '<tr><td><a href="viewacct?TID='.$cid.'" target="_blank"><i class="fa fa-eye"></i></a> <em>'.$AccTClass.'</em></td><td>-</td><td>'.  rmvneg($DisAmt).'</td><td>&nbsp;</td></tr>';
...
function getTotalAmt($MasterID)
{
    ...
    $FromD = $GLOBALS['FromD']; 
   $ToD = $GLOBALS['ToD'];
  $resultChartMaster = mysql_query("SELECT *, acc_chart_class.cid AS ACLASS FROM postings1
             ...
             WHERE postings1.GLImpacted = $MasterID AND postings1.isActive=1
             ");
                ...
                <div class="box-header with-border">
                  ...
                  <div class="row">
                    <div class="col-md-12">
                        ...
                        <form role="form" action="balancesheet1" method="POST" ><div class="form-group">
