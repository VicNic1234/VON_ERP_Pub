            ...
            $AGETAB .= '<tr>
                               ...
                               <td>'.number_format($TFINAL, 2).'</td>
                                ...
                                <td>'.number_format($AMTPOSTED[4], 2).'</td>
                                 <td>'.number_format($AMTPOSTED[5], 2).'</td>
                                  <td>'.number_format($AMTPOSTED[6], 2).'</td>
                              </tr>';
            }
        }
    }
...
function getTotalAmtPosted($VenID)
{
   ...
   $resultPOSTING = mysql_query("SELECT * FROM postings WHERE
   (postings.GLImpacted != '602' AND postings.TransacType = 'CREDIT' AND postings.VendorID = '$VenID' AND postings.isActive=1 AND postings.TransactionAmount > 0  )   
...
if ($NoRowPOST > 0) {
  ...
  {
    $PayBal = $row['TransactionAmount']; $TranAmt = $row['TransactionAmount'];
    $TT = $row['TransacType'];
    $IVDate = (str_replace("/", "-", $row['TransactionDate']));
    ...
    //$ENLINVOICE = $row['VINVOICE'];
      ...
      { 
            ...
            if($FromD == "")
                {
                     //Let's try to get Total Debit and Total Payment
                     if($TT =="DEBIT")  { $TotalDebit += $TranAmt; } 
                     if($TT =="CREDIT") { $TotalPayment += $TranAmt; } 
                     //////////////////////////////////////////////////////
                    if($DateDiff < 31) { $D0T30 += floatval($PayBal); }
                    if($DateDiff > 30 && $DateDiff < 61) { $D31T60 += floatval($PayBal); }
                    if($DateDiff > 60 && $DateDiff < 91) { $D61T90 += floatval($PayBal); }
  ...
  //At this point we need to cancel out before we push it out
  
  return array($PostedAmtMOK, $D0T30, $D31T60, $D61T90, $D91UP, $TotalDebit, $TotalPayment);
