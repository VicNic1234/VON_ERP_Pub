...
?>      
    ...
    <div class="row">
      ...
      <div class="form-group has-feedback col-md-1" title="Get All Payable Invoices">
         <label> &nbsp;</label>
    ...
    <div class="form-group has-feedback col-md-1">
     ...
     <div class="form-group col-md-2"><label>Transaction Date: </label><input type="text" class="form-control" id="TRANSDATE" name="TRANSDATE" onInput="setBankGL();" readonly required /></div>
    ...
    <script type="text/javascript">
      ...
      function LoadVenPON()
      {
        ...
        var VenID = $('#VendorID').val(); 
        if(VenID == "") { alert('Kindly select Vendor'); return false; }
        if(InvoiceNum == "") { alert('Kindly select an INVOICE NUMBER'); return false; }
        var formData = {VenID:VenID, InvoiceNum:InvoiceNum };
...
function noDuctions()
{
  ...
  var TotalPayableVal = 0.0;
   ...
   $(".applychk").each(function( index ) {
              ...
              //Let's setup a fat tracker now
              //var formtoPost '<form ></form>'

    var setAcctTabFormat = '<tr class="classicman" me="516">'+
                '<td>[Trade Payable] '+VenNAME+'</td>'+ 
                '<td id="TPDD"></td>'+ 
                '<td>-</td>'+
                '</tr>';
                ...
                //alert("No Deductions");
                //alert(TotalPayableVal.toFixed(2));
                $('#TPDD').html(formatn(TotalPayableVal.toFixed(2)));
                //$('#TPDDV').html(formatn(TotalPayableVal.toFixed(2)));
...
function removeCOM(elem)
{
  ...
  //if(CURRC == "") { alert('Kindly select Currency'); return false; }
        ...
        function setBankGL()
        {
          ...
          //alert(SELEBNKGL);
          var GLID = $('#BANKGL option:selected').attr("glacc");
          //alert(GLID);
          $('#bnkGL').html($('#BANKGL option:selected').text());
          $('#bnkGLVal').val(GLID);
          $('#bnkID').val($('#BANKGL').val());
      ...
      function postBankPay()
        {
           ...
           var OptGLBank = '<?php echo $OptGLBank; ?>';
            ...
            var size='standart';
            var HLTab = $('#HTabBody').html();
            if(HLTab == "")
            {
              //JESUS IS GREATER
              //alert("JESUS IS GREATER");
              noDuctions();
            }
            ...
            BankCreditBal = BankCreditBal.toFixed(2);
             ...
             var RowHTML = '<tr>'+
                          '<td id="bnkGL">Bank</td>'+ 
                          '<td> <input type="hidden" id="bnkID" value=""  /> <input type="hidden" id="bnkGLVal" value=""  /> - </td>'+ 
        ...
        function nowPost()
        {
             ...
             var InvoiceNo = $('#InvoiceNumV').html();  
             ...
             //var formData = {BankGL:BnkChkGL};
                    ...
                    $.ajax({
                        url: "../utility/bnkpaypost?bnkgl="+BnkChkGL+"&bnkamt="+bnkAmt+"&bnkchq="+BANKCHQ+"&bnktdate="+TRANSDATE+"&bnkid="+BnkID+"&invno="+InvoiceNo,
                        type: "POST",
...
<div id="PrintArea" >
        ...
        <img id="pelogo" src="../mBOOT/plant.png" width="100px" height="80px" alt="PEEL logo"/>
         ...
         <div class="row">
          <span id="InvoiceNumV" style="color:#FFF"></span>
          ...
          <div class="row">
            ...
            <table id="AccTabBodyV" class="table table-striped" style="display:none"></table>
            <table id="AccTabBody" class="table table-striped" style="display:none"></table>
          </div>
