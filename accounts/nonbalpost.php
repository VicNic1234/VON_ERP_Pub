...
function ChkIfBal($tncid)
{
    ...
    $resultChartMaster = mysql_query("SELECT TransactionAmount, TransacType, GLImpacted FROM postings  WHERE 
    (postings.tncid = '$tncid' OR CounterTrans = '$tncid')   
            ...
            if ($NoRowChartMaster > 0) {
              while ($row = mysql_fetch_array($resultChartMaster)) {
                ...
                $TT = $row['TransacType']; //ClassName$TCRD
                $TDate = $row['TransactionDate']; $GLImpacted = $row['GLImpacted'];
          ...
          return ($TCRD - $TDRD);  // return ($TDRD - $TCRD); 
}
