...
$resultLI = mysql_query("SELECT *, 
 ...
 LEFT JOIN suppliers ON acct_vendorsinvoices.VendSource = suppliers.supid
  ...
  WHERE acct_vendorsinvoices.isActive=1 AND PostID > 0 AND suppliers.supid = 121  GROUP BY acct_vendorsinvoices.VendSource  ORDER BY acct_vendorsinvoices.cid
 "); //GROUP BY acct_vendorsinvoices.VendSource //LEFT JOIN users ON acct_vendorsinvoices.RaisedBy = users.uid //121
...
if ($NoRowLI > 0) {
  while ($row = mysql_fetch_array($resultLI)) 
  {
    ...
    $cid = $row['cid'];
    $supid = $row['supid'];
    $SupName = $row['SupNme']; // echo "<br/>";
    //We may need to do a bypass - CHECK if the person is owed at all
    //Total Payable Credit 
       /////////////////////////////////////////////////////////////
      $resultPOSTING = mysql_query("SELECT SUM(TransactionAmount) AS TPAC FROM postings 
   Where postings.VendorID='$supid' AND (postings.TransacType='CREDIT' AND postings.GLImpacted = '602') AND postings.isActive=1"); //GROUP BY postings.VINVOICE
 ...
 //Total Payable Debit 
       /////////////////////////////////////////////////////////////
      $resultPOSTING = mysql_query("SELECT SUM(TransactionAmount) AS TPAD FROM postings 
   Where postings.VendorID='$supid' AND (postings.TransacType='DEBIT' AND postings.GLImpacted = '602') AND postings.isActive=1"); //GROUP BY postings.VINVOICE
  $NoRowPOST = mysql_num_rows($resultPOSTING); 
 if ($NoRowPOST > 0) 
 {
  while ($row = mysql_fetch_array($resultPOSTING)) 
 ...
 echo floatval($TPAC) - floatval($TPAD); exit;
  //////////////////////////////////////////////////////////
   // echo $AMTPOSTED = getTotalAmtOwed($supid)[0]; exit; //
...
function getTotalAmtOwed($VenID)
{
   ...
   $ToD = $GLOBALS['ToD'];
   $PostedAmt = 0.0; $D0T30 = 0.0; $D31T60= 0.0; $D61T90 = 0.0; $D91UP = 0.0;
  /// LIST ALL CREDIT INVOICES FOR THIS SUPPLIER/////
  //$resultINLIST = mysql_query("SELECT * FROM postings 
   //LEFT JOIN acct_vendorsinvoices ON postings.VINVOICE = acct_vendorsinvoices.cid
  // Where postings.VendorID='$VenID' AND (postings.TransacType='CREDIT' AND postings.GLImpacted = '602') AND postings.isActive=1");
   ...
   $resultOWE = mysql_query("SELECT * FROM postings 
   LEFT JOIN acct_vendorsinvoices ON postings.VINVOICE = acct_vendorsinvoices.cid
...
$NoRowPOST = mysql_num_rows($resultOWE);
 if ($NoRowPOST > 0) 
 {
  while ($row = mysql_fetch_array($resultOWE)) 
  {
    $pamt = $row['TransactionAmount'];
    $IVDate = (str_replace("/", "-", $row['IVDate']));
    //$IVD = date_create($IVDate);
    ...
    ////////////////// SO IF THERE IS A CREDIT FOR THIS INVOICE DO THIS /////////////////////////////////
    $PaidAmt = getTotalAmtPaid($VINVOICE);
    $PayBal = floatval($pamt)  - floatval($PaidAmt);
    if($PayBal > 0)
    {
  ...
  */
  return array($PostedAmt, $D0T30, $D31T60, $D61T90, $D91UP);
  //return array($VINVOICE, $D0T30, $D31T60, $D61T90, $D91UP);
  //return $DateDiff;//$PostedAmt - $PaidAmt;
}
