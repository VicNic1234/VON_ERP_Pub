...
$TCRD = 0.0; $TDRD = 0.0; $CHKBAL = 0.0;
    $resultChartMaster = mysql_query("SELECT *, postings.tncid As TNID, enlinvoices.cid As ivcid, reciepts.CheuqeNME As CHKNME  FROM postings
            ...
            elseif ($FromD <= $TD && $TD <= $ToD) 
            {
                if($TT =="CREDIT")  { $DRD = $TAmount; $TDRD = $TDRD + $TAmount; $CHKBAL = $CHKBAL + $DRD; } else { $DRD = ''; }
                if($TT =="DEBIT") { $CRD = $TAmount; $TCRD = $TCRD + $TAmount; $CHKBAL = $CHKBAL - $CRD; } else { $CRD = ''; }
              $RecChartMaster .= '<tr><td>'.$acid.'</td><td><a href="viewacct?TID='.$cid.'"><i class="fa fa-eye"></i></a> '.$name.'</td><td>'.$TD.'</td><td>'.$invoclinke.' '.$enlivNo.'</td><td>'.$CheuqeNME.'</td><td><a href="viewacct?ACCTID='.$mid2.'"><i class="fa fa-eye text-green"></i></a> '.$mid3.'</td><td>'.$TDescr.'</td><td id="dr-'.$RowID.'">'.number_format($DRD, 2).'</td><td id="cr-'.$RowID.'">'.number_format($CRD, 2).'</td>
     ...
     $RecChartMasterOB .= '<tr><td>(Opening Balance)</td><td>(Opening Balance)</td><td>(Opening Balance)</td><td>(Opening Balance)</td><td>(Opening Balance)</td><td>(Opening Balance)</td><td>(Opening Balance)</td><td id="dr-1" >'.number_format($DRDOB,2).'</td><td id="cr-1" >'.number_format($CRDOB,2).'</td>
                ...
                <div class="box-header with-border">
                  ...
                  <div class="row">
                    <div class="col-md-12">
                        ...
                        <form role="form" action="cusGLY" method="POST" ><div class="form-group">
