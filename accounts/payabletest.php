...
if ($NoRowLI > 0) {
  while ($row = mysql_fetch_array($resultLI)) 
  {
   ...
   // $amtOweing = ChkIfOwening($cusid);
    ...
    //if($amtOweing > 0)
    {
        $AMTPOSTED = getTotalAmtPosted($supid);// echo $AMTPOSTED[0];
         //New Algorithm to net out per customer
         
         $TotalAmtPaidArry = getTotalAmtPaid($supid);
         $TotalAmtPaid = $TotalAmtPaidArry[0];
         // TAP ON OVER PAY 
         if($AMTPOSTED[0] < $TotalAmtPaid) { 
            // $TotalAmtPaid = $AMTPOSTED[0];  
            $SETOVERPAYAMT += $TotalAmtPaid - $AMTPOSTED[0];
             
         }
         
         $FTotalAmtPaid = $TotalAmtPaid;
       ...
       // exit;
         ...
         */
         ...
         ////////////////////////////////////////////////////////////////////////////////
         if($TotalAmtPaidArry[0] > 0 )
           {
             //$FTotalAmtPaid = $FTotalAmtPaid - $AMTPOSTED[4];
             ///We need to change this part to colliding deduction
             
             $AMTPOSTED[4] = $AMTPOSTED[4] - $TotalAmtPaidArry[4];
             $AMTPOSTED[3] = $AMTPOSTED[3] - $TotalAmtPaidArry[3];
             $AMTPOSTED[2] = $AMTPOSTED[2] - $TotalAmtPaidArry[2];
             $AMTPOSTED[1] = $AMTPOSTED[1] - $TotalAmtPaidArry[1];
             
             
             ////////// new algorithm
            /* $FTotalAmtPaid = $TotalAmtPaidArry[5]; // $TotalAmtPaidArry[4] + $TotalAmtPaidArry[3] + $TotalAmtPaidArry[2] + $TotalAmtPaidArry[1];
             
             
               if($FTotalAmtPaid > 0 )
               {
                 $FTotalAmtPaid = $FTotalAmtPaid - $AMTPOSTED[4]; // -898
                 if($FTotalAmtPaid >= 0) { $AMTPOSTED[4] = 0;  } else { $AMTPOSTED[4] = ABS($FTotalAmtPaid); }
         ...
         $D91UP += $AMTPOSTED[4];
        $TotalAmtPaidGP = 0.0;
        //if($TotalAmtPaid > $AMTPOSTED[0]) { $TotalAmtPaidGP = '<b style="color:red">'. number_format($TotalAmtPaid, 2) . '</b>'; } 
       // else { $TotalAmtPaidGP =  number_format($TotalAmtPaid, 2) ; }
        // <td>'.number_format($AMTPOSTED[0], 2).'</td>
        // <td>'.$TotalAmtPaidGP.'</td>
            if($TFINAL > 0)
            {
            $AGETAB .= '<tr>
  ...
  //  <th>'.number_format($TotalAmtPaidMAX, 2).'</th>
        ...
        if($SETOVERPAYAMT > 0)
        {
            $AGETAB .= '<tr style="color:red">
                        <td>Over Payment</td>
                        <th> <b>'.number_format($SETOVERPAYAMT, 2).'</b></th>
                      
                        <th>'.number_format($D0T30 - $SETOVERPAYAMT, 2).'</th>
                        <th>'.number_format($D31T60, 2).'</th>
                        <th>'.number_format($D61T90, 2).'</th>
                        <th>'.number_format($D91UP, 2).'</th>
                      </tr>';
            ...
            $AGETAB .= '<tr>
                        <td>Grand Total</td>
                        <th>'.number_format($PostedAmtMAC - $SETOVERPAYAMT, 2).'</th>
...
function getTotalAmtPaid($VenID)
{
 ...
 // $resultPAID = mysql_query("SELECT * FROM postings Where VendorID='$VenID' AND TransacType='DEBIT' AND GLImpacted='602' AND postings.isActive=1");
  $resultPAID = mysql_query("SELECT * FROM postings Where VendorID='$VenID' AND TransacType='CREDIT' AND GLImpacted<>'602' AND postings.isActive=1");
        ...
        //return $PaidAmt;
        return array($PostedAmtMOK, $D0T30, $D31T60, $D61T90, $D91UP, $GTOTAL);
}
...
function getTotalAmtPosted($VenID)
{
   ...
   // $resultPOSTING = mysql_query("SELECT * FROM postings Where postings.VendorID='$VenID' AND postings.TransacType='DEBIT' AND postings.GLImpacted<>'602' AND postings.isActive=1
   //ORDER BY TransactionDate");
                ...
                <div class="box-header with-border">
                  ...
                  <div class="row">
                   <div class="col-md-12">
                        ...
                        <form role="form" action="payabletest" method="POST" ><div class="form-group">
                ...
                <div class="box-body">
                   <!--<table id="userTab" class="table table-bordered table-striped">
                    <thead>
                        ...
                        <tr>
                            <th colspan="3"><center style="font-size:1.3em"> Payable Aging as at <?php echo date("Y F"); ?> </center> </th>
                        </tr>
                      <tr>
                        <th>--</th>
                        <th>Notes</th>
