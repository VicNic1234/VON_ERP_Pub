...
//(postings.GLImpacted <> 602 AND TransacType = 'CREDIT' AND postings.VendorID = $VendSource )   OR (GLDescription =  'Trade Payable' AND postings.VendorID = $VendSource)
   ...
   //Let's Read ChartType
          ...
          /*$resultChartMaster = mysql_query("SELECT *, postings.tncid As TNID, acct_vendorsinvoices.cid As ivcid, cheuqes.CheuqeNME As CHKNME FROM postings
             ...
             WHERE (postings.GLImpacted <> 602 AND TransacType = 'CREDIT' AND postings.VendorID = '$VendSource' AND postings.isActive=1 )   OR (GLDescriptionPB =  'Trade Payable' AND postings.VendorID = '$VendSource' AND postings.isActive=1) 
             ...
             WHERE (postings.GLImpacted = 602 AND TransacType = 'CREDIT' AND postings.VendorID = '$VendSource' AND postings.isActive=1 ) 
            ...
            if ($NoRowChartMaster > 0) {
              while ($row = mysql_fetch_array($resultChartMaster)) {
               ...
               if($FromD == "")
            {
                $RecChartMaster .= '<tr><td>'.$acid.'</td><td><span onClick="javascript:popUp(\'viewacct?TID='.$cid.'\')" ><i class="fa fa-eye"></i></span> '.$name.'</td><td>'.$TD.'</td><td>'.$invoclinke.' '.$vendivNo.'</td><td>'.$CheuqeNME.'</td><td><a href="viewacct?ACCTID='.$mid2.'"><i class="fa fa-eye text-green"></i></a> '.$mid3.'</td><td>'.$TDescr.'</td><td>'.number_format($DRD,2).'</td><td>'.number_format($CRD,2).'</td>
            ...
            elseif ($FromD <= $TD && $TD <= $ToD)
            {
             ...
             WHERE (postings.GLImpacted = 602 AND TransacType = 'DEBIT' AND postings.VendorID = '$VendSource' AND postings.isActive=1 ) 
                ...
                <div class="box-header with-border">
                  ...
                  <div class="row">
                    <div class="col-md-12">
                        ...
                        <form role="form" action="venGLY1" method="POST" ><div class="form-group">
                ...
                <div class="box-body">
                   ...
                   <table id="userTab" class="table table-bordered table-striped">
                       ...
                       <tr>
                          ...
                          <td colspan="10"><center style="font-weight:800; font-size:1.2em">Vendor - <?php echo $VenNme; ?>'s Ledger</center></td>
                      </tr>
                       </thead>
                    <thead>
                      <tr>
