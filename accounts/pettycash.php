...
include('route.php');
 $PageTitle = "MD Approved Cash Request";
    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
            <i class="fa fa-money text-green"></i> Petty Cash
...
<?php
 ...
 LEFT JOIN users ON cashreq.staffID = users.uid where cashreq.isActive=1 AND cashreq.Approved=11"); //WHERE isActive=1 ORDER BY cid WHERE poreq.Approved = 11
...
//fetch tha data from the database
	 if ($NoRowLI > 0) 
   {
    ...
    $Purpose = $row['Purpose'];
    $Amount = $row['Amount'];
    $AAmount = number_format($row['ApprovedAmount']);
    $Qty = $row['Qty'];
    if($Qty == "") { $Qty = 1; }
    $Amount = number_format(floatval($Qty) * floatval($Amount), 2);
	  $Statun = $row['Status'];
    ...
    if($FileLink != ""){
    $FileLinkn = '<a target="_blank" title="View Status" href="'.$FileLink.'"><i class="fa fa-link"></i></a>';
    }
    else
    {
      $FileLinkn = '';
    ...
    $ViewCon = '<a title="View Status" href="../requisition/cashviewacct?rNo='.$RequestID.'"><i class="fa fa-eye"></i></a>';
