   ...
   //Let's Read ChartType
      ...
      if ($NoRowType > 0) {
        while ($row = mysql_fetch_array($ChartTypeQ)) {
          ...
          //////////////////////////////////////////////////////////////////////////
        if($TID != ""){
          $resultChartMaster = mysql_query("SELECT *, postings1.ChqNo As ChqNoM, postings1.tncid As TNCID FROM postings1
             LEFT JOIN acc_chart_master ON postings1.GLImpacted = acc_chart_master.mid
             LEFT JOIN cheuqes ON postings1.ChqNo = cheuqes.chid
             WHERE acc_chart_master.account_type = $tid && postings1.GLImpacted = $TID 
             ORDER BY acc_chart_master.mid, postings1.TransactionDate");
        }
        //&& postings1.isActive=1
        if($ACCTID != ""){
          $resultChartMaster = mysql_query("SELECT *, postings1.ChqNo As ChqNoM, postings1.tncid As TNCID, postings1.isActive As PA  FROM postings1
             LEFT JOIN acc_chart_master ON postings1.GLImpacted = acc_chart_master.mid
             LEFT JOIN cheuqes ON postings1.ChqNo = cheuqes.chid
             WHERE acc_chart_master.account_type = $tid  && (postings1.tncid = '$ACCTID' OR postings1.CounterTrans = '$ACCTID')
             ORDER BY acc_chart_master.mid, postings1.TransactionDate");
        }
            ...
            if ($NoRowChartMaster > 0) {
              while ($row = mysql_fetch_array($resultChartMaster)) {
                ...
                if($ACCTID > 0)
                {
                $DelBtn = '<a href="delacct1?p='.$TNCID.'"><span title="Delete Posting" class="btn btn-danger"><i class="fa fa-trash"></i></span></a>';
         ...
         function updateVAL(tnic)
         {
            ...
            success: function(html)
            {
                     ...
                     var OptStaff = '<?php echo $OptStaff; ?>';
                      ...
                      var size='standart';
                  var content = '<div class="row" style="background:#EEE6E6; border-radius:5px;" > <div class="col-md-12"><form role="form" action="updatepost1" method="post" ><div class="form-group">' +
