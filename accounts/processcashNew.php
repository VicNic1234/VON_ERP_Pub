...
$LPD = $_SESSION['LastPostDay'];
if($LPD == "") { echo "Kindly <a href='../users/logout'>Logout</a> and Login again. Thanks"; exit; }
$msg = "";
if ($NoRowMsg > 0) 
{
    ...
    if ($NoRowBnkSet > 0) 
      {
        while ($row = mysql_fetch_array($resultGLBnk)) 
        {
          ...
          $GLAcct = $row['GLAcct'];
          $acctval = mysql_real_escape_string($row['account_name']);
          $OptGLBank .='<option glacc="'.$GLAcct.'" value="'.$acctid.'">['.$acctvariable.'] '.$acctval.' :: '.$acctbnkDesp.'</option>';
        }
      }

//Let's Get All GL accounts
 $GLAccounts = mysql_query("SELECT * FROM acc_chart_master LEFT JOIN bankaccount ON acc_chart_master.mid = bankaccount.GLAcct WHERE acc_chart_master.isActive =1 ORDER BY account_name"); 
 while ($row = mysql_fetch_array($GLAccounts)) {
      $bacctid = $row['baccid'];
      $acctbnkDesp = $row['description'];
     $acctvariable = $row['account_code'];
     $GLmid = $row['mid'];
      $GLAcct = mysql_real_escape_string($row['account_name']);
      if($GLmid == 602)
      {
       $GLOpt .= '<option value="'.$GLmid.'" selected >['. $acctvariable .'] ' .$GLAcct.'</option>'; // $row['DeptmentName'];
       //$GLOptRT .= '<option value="'.$GLmid.'" selected >['. $acctvariable .'] ' .$GLAcct.'</option>';
      }
      else
      {
      ...
      if($GLmid == 720)
      {
       $GLOptV .= '<option value="'.$GLmid.'" selected >['. $acctvariable .'] ' .$GLAcct.' :: '.$acctbnkDesp.'</option>'; // $row['DeptmentName'];
      }
      else
      {
          $GLOptV .= '<option bnk="'.$bacctid.'" value="'.$GLmid.'">['. $acctvariable .'] ' .$GLAcct.' :: '.$acctbnkDesp.'</option>'; // $row['DeptmentName'];
      }
...
//Let's Get All Equipment
 $EquipCT = mysql_query("SELECT * FROM equipments WHERE isActivate =1 ORDER BY EquipNme"); 
 while ($row = mysql_fetch_array($EquipCT)) {
     //$reqid = $row['reqid'];
     ...
     //Let's Get All Jobs
 $EquipJob = mysql_query("SELECT * FROM jobs WHERE isActive =1"); 
 while ($row = mysql_fetch_array($EquipJob)) {
     //$reqid = $row['reqid'];
     $JIB = $row['chid'];
    
      $JNME = $row['cheuqeNME'];
       $JobOpt .= '<option value="'.$JIB.'">'.$JNME.'</option>'; // $row['DeptmentName'];
     }
  ...
  <body class="skin-blue sidebar-mini">
    ...
    <?php include('leftmenu.php') ?>
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">Process Bank Payments</li>
          </ol>
        </section>
                 ...
                 function loadChk(elem)
     {
        ...
        $.ajax({
            ...
            error: function(a,b,c)
            {
              ...
              function loadChkFrmBNKID(BNKID)
     {
     ...
     function setDescription()
     {
        ...
        $('#payerDEC').html("Bank to be Credited");
...
'<div class="alert alert-info alert-dismissable">' .
									...
									$_SESSION['ErrMsgB'] = "";}
?>
  <form id="postPetty" action="postPettyNew" method="POST">
          <div class="row">
            <div class="col-md-12">
              <div class="box <?php if($DDOfficerOn != "") { echo 'collapsed-box' ; } ?>">

                <div class="box-header with-border">
                  <h3 class="box-title">Cheque/Ref No. Details</h3>
      ...
      var OptGLBank = '<?php echo $OptGLBank ?>'; 
         ...
         <div class="form-group has-feedback col-md-3">
          ...
          <!--<div class="form-group has-feedback col-md-3">
           ...
           <input type="text" class="form-control" name="TAMT" id="TAMT" onKeyPress="return isNumber(event)" oninput="setBNKAMT()" onchange="setBNKAMT()" />
            <span class="glyphicon glyphicon-cash form-control-feedback"></span>
          </div>
          <script>
              function setBNKAMT()
              {
                  var TAMT = $('#TAMT').val(); $('#PostingTMT').val(TAMT);
                  //alert(TAMT);
                  var CAmt = addCommas(TAMT);
                    // $('#TAMT').val(Amt);
                     $('#rDCredit').html(CAmt);
                     $('#rDCredit').attr("amt", TAMT); resetBal();
              }
          </script>-->
          ...
          <input type="hidden" id="payerDECV" name="payerDECV" />
                     ...
                     <input type="hidden" name="TAmt" id="PostingTMT" value="" />
              ...
              <div class="box box-warning">
                <div class="box-header with-border">
                  <center><h4> CASH REQUITIONS AND OTHER PAYMENTS <b>[Debit GL]</b> </h4></center> 
                   <div class="box-tools pull-right">
                    <button class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
                ...
                <div class="box-body">
                  <div class="row" style="background-color: #A3CDB5; padding:12px; border-radius:15px">
                    <div class="col-md-12">

                      <div class="col-md-6">
                        <div class="form-group col-md-6">
                          <label>Debiting GL Account: </label>
                          <select class="select2 form-control" style="width: 100%" id="DBGL" name="DBGL" required ><option value=""> --- </option><?php echo $GLOpt; ?></select>
                        </div> 
                        ...
                        <div class="form-group col-md-6">
                          ...
                          <select class="select2 form-control" style="width: 100%" id="mJob" name="mJob" ><option value=""> --- </option>
                          <?php echo $JobOpt; ?>
                          </select>
                        </div> 
                         <div class="form-group has-feedback col-md-6">
                          <label>Equipment:</label>
                          <select class="form-control select2" id="EquipC" name="EquipC"  >
                          <option value="">-- </option>
                          <?php echo $EquipOpt; ?>
                          </select>
                        </div>
                      ...
                      <div class="col-md-6">
                        ...
                        <div class="form-group col-md-6">
                          ...
                          <select class="select2 form-control" style="width: 100%" id="VEND" name="VEND" onchange="chkVEN(this)" required ><option value=""> --- </option><?php echo $SupOpt; ?></select>
                        </div> 
                        <div class="form-group col-md-6 vndctrl" style="display:none">
                             <label> &nbsp; &nbsp; </label>
                          <button type="button" class="btn btn-primary form-control" onclick="addLIVEN()"> Add Item From Vendor's Invoice </button>
                        </div>
                        
                        <div class="form-group col-md-12 nvndctrl">
                       ...
                       <script>
                           function chkVEN(elem)
                           {
                              var Vend = $(elem).val(); if(Vend > 0) { $('.vndctrl').show(); $('.nvndctrl').hide();  } else { $('.nvndctrl').show(); $('.vndctrl').hide(); }
                           }
                        </script>
              ...
              <div class="box box-warning">
                <div class="box-header with-border">
                  <center><h4> BANKS, WHT AND TAXS <b>[Credit GL]</b> </h4></center> 
                   <div class="box-tools pull-right">
                    <button class="btn btn-box-tool" data-widget="collapse"><i class="fa fa-minus"></i></button>
                ...
                <div class="box-body">
                  <div class="row" style="background-color: #EEC9C1; padding:12px; border-radius:15px">
                    <div class="col-md-12">

                      
                        <div class="form-group col-md-4">
                         <label>Crediting GL Account: </label>
                          <select class="select2 form-control" style="width: 100%" id="DBGLV" name="DBGLV" onchange="ShowBNK(this);" required ><option value=""> --- </option><?php echo $GLOptV; ?></select>
                        </div> 
                        <div class="form-group has-feedback col-md-6 bnkchq"  style="display:none">
                                <label>Select Cheque/Ref No.:</label>
                                ...
                                <div class="input-group">
                                   <div class="input-group-addon" onclick="RegChk()" style="background-color: green; color:#FFF">
                                      ...
                                      <select class="select2 form-control" style="width:100%" onchange="getCHK(this)" id="ChqNo" name="ChqNo" required>
                                      <option value=""> Choose Cheque/Ref No.</option>
                                
                                </select>
                                </div>
                        </div>
                         <div class="form-group has-feedback col-md-3 bnkchq" style="display:none">
                            <label>Total Amount:</label>
                           <input type="text" class="form-control" name="TAMT" id="TAMT" onKeyPress="return isNumber(event)"  readonly /> <!-- oninput="setBNKAMT()" onchange="setBNKAMT()" -->
                            <span class="glyphicon glyphicon-cash form-control-feedback"></span>
                         </div>
                          <div class="form-group has-feedback col-md-3 bnkchq" style="display:none">
                            <label style="color:#EEC9C1">...</label>
                            <input type="hidden" id="BANKGLN" />
                           <button type="button" onclick="addBNKCHQ();" class="btn btn-success form-control"> Add Bank Chq/Transfer </button>
                         </div>
                        ...
                        <script>
                            function ShowBNK(elem)
                            {
                                var bnk = $(elem).find(':selected').attr('bnk');
                                $('#BANKGLN').val(bnk);
                                if(bnk > 0)
                                { $('.nbnk').hide(); $('.bnkchq').show(); loadChkFrmBNKID(bnk); }
                                else
                                { $('.nbnk').show(); $('.bnkchq').hide(); }
                            }
                        </script>
                       
                        <div class="form-group col-md-2 nbnk">
                            <label style="color:#EEC9C1"> .....</label>
                          <button type="button" class="btn btn-warning form-control" onclick="addLIV()"> Add Item </button>
                        </div>
                       
                        <div class="form-group col-md-4 nbnk">
                         <label style="color:#EEC9C1"> .....</label>
                           <button type="button" class="btn btn-primary form-control" onclick="addLIVENV()"> Add Item From Vendor's Invoice </button>
                        </div>
              ...
              <div class="box box-success">
                ...
                <div class="box-body">
                     ...
                     <tr><th>Description</th><th>GL Impacting</th><th>Debit </th><th>Credit</th></tr> 
                      ...
                      <td id="rDCredit" class="pCredit" amt="0">0.00</td>
                      </tr>-->
                      <tr id="BNKCASH">
                      <td colspan="4" id="payerDEC"><center>---</center></td>
		...
		<script>
         ...
         var DBNUM = $('#DBNUM').val();
         //////////////////////////////////////////////////////////////////////////
        var BANKGL = $('#BANKGLN').val();
        var ChqNo = $('#ChqNo').val();
        var transactiondate = $('#transactiondate').val();
        var RECEIVER = $('#RECEIVER').val();
        var TCurr = $('#TCurr').val();
        var DBGL = $('#DBGLV').val();
        var DBGLt = $('#DBGLV :selected').text();
        var BusUnit = $('#BusUnit').val();
        ...
        ////////////////////////////////////////////////////////////
        if(ChqNo == "") { alert("Kindly select Cheque or Transfer ID"); return false; }
        
        var CHRID = $('#CusID').val(); //if(CHRID == "") { alert("Kindly select Customer"); return false; }
       
        var CHRNum = $('#CusInv').val(); //if(CHRNum == "") { alert("Kindly select ENL Invoice to this Vendor"); return false; }

        var CHSItem = $('#INVItem').val(); //if(CHSItem == "") { alert("Kindly select Invoice Item"); return false; }

        var CHSItemDesc = "Bank to be Credited";//$('#INVItemDesc').val(); 
        if(CHSItemDesc == "") { alert("Kindly enter Description"); return false; }

        var CHSUnitQty = 1;//$('#INVUnitQty').val(); if(CHSUnitQty == "") { alert("Kindly enter Invoice item Quantity"); return false; }

        var CHSUnitPrice = $('#TAMT').val();//$('#INVUnitPrice').val(); if(CHSUnitPrice == "") { alert("Kindly enter Invoice Item Amount"); return false; }

        var CHSTotalPrice = $('#TAMT').val(); if(CHSTotalPrice == "") { alert("Kindly enter Invoice Item Amount"); return false; }
        
        var iVEND = $('#VEND').val(); /* alert(iVEND);*/ var wvVENDid =  $("#VEND option:selected").text(); //alert(wvVENDid);
       // if(iVEND == "") { alert("Kindly select Vendor"); return false; }
         /////////////////////////////////////////////////////////////////////////////
        ...
        //alert("Debit Test");
          ...
          var TRow =   '<tr>'+
                     '<td class="receiveDEC">'+
                    CHSItemDesc+
                     '<br/> <em style="font-size:13px; color:green; font-weight:800"> '+wvVENDid+' </em> </td>'+

                     '<td class="receiveGL">'+
                      '<input type="hidden" name="CrDesc[]" value="'+DBGLt+'@&@'+DBGL+'@&@'+CHSTotalPrice+'@&@'+BusUnit+'@&@'+EquipC+'@&@'+CHSItemDesc+'@&@'+CHSUnitQty+'@&@'+CHRNum+'@&@'+CHSItem+'@&@VENDINV@&@'+CHRID+'@&@'+mItem+'@&@'+mJob+'@&@'+RECEIVER+'@&@'+iVEND+'" />'+
                     DBGLt+
                     '</td>'+
                     

                     '<td>0.00</td>'+
                      '<td class="pCredit" amt="'+CHSTotalPrice+'">'+addCommas(CHSTotalPrice)+'</td>'+
             ...
             */
             ...
             //$('#EntryChart > tbody:last-child').append(TRow);
             $('#BNKCASH').after(TRow);
             resetBal();
 ...
 <script>
    ...
    <script type="text/javascript">
          function addLIV()
      {
        ...
        var ChqNo = $('#ChqNo').val();
        //if(ChqNo == "") { alert("Kindly select Cheque Number. Thanks"); return false; }
        ...
        var UOMOpt = '<?php echo $UOMOpt; ?>';
          var size='standart';
                  var content = '<form role="form" enctype="multipart/form-data" ><div class="form-group">' +
                   ...
                   '<button type="button" onclick="addBDV()" class="btn btn-success pull-right">Add Item</button><br/></form>';
    ...
    <script type="text/javascript">
      function addLIVENV()
      {
        ...
        if(VEND == "") { alert("Kindly select Vendor. Thanks"); return false; }
        ...
        /////////////////////////////////////////////////////////////
         ...
         var size='standart';
                  var content = '<div class="row" style="background:#EEE6E6; border-radius:5px;" > <div class="col-md-12"><form role="form" ><div class="form-group">' +
                    ...
                    '<div class="form-group col-md-6"><label>Select Vendor\'s Posted  Invoice: </label><select class="form-control" onchange="setInvItems(this)" id="VenInv" name="VenInv" required ><option value=""> --- </option>'
                    +'</select></div>' +
                    ...
                    '<input type="hidden" id="mItem" value="0" />'+
                   ...
                   '<button type="button" onclick="addCDINV()" class="btn btn-success pull-right">Add Item</button><br/></form></div></div>';
                  var title = 'Vendor Invoice Item Details';
                  var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>';
    ...
    <script>
      ...
      function addBDINV()
     {
        ...
        var CHRID = $('#INVID').val(); if(CHRID == "") { alert("Kindly select Vendor"); return false; }
       
        var CHRNum = $('#VenInv').val(); if(CHRNum == "") { alert("Kindly select Invoice of this Vendor"); return false; }
         ...
         // { alert('Debit Amount must balance on table'); return false; }
          ...
          var TRow =   '<tr>'+
                    ...
                    CHSItemDesc+
                     ...
                     '<td class="receiveGL">'+
                      '<input type="hidden" name="DrDesc[]" value="'+DBGLt+'@&@'+DBGL+'@&@'+CHSTotalPrice+'@&@'+BusUnit+'@&@'+EquipC+'@&@'+CHSItemDesc+'@&@'+CHSUnitQty+'@&@'+CHRNum+'@&@'+CHSItem+'@&@VENDINV@&@'+CHRID+'@&@'+mItem+'@&@'+mJob+'@&@'+RECEIVER+'@&@'+iVEND+'" />'+
                     DBGLt+
                     '</td>'+
      ...
      function addBDCHS()
     {
        ...
        var EquipC = $('#EquipC').val();
         var mItem = $('#mItem').val();
        var mJob = $('#mJob').val();
        var iVEND = $('#VEND').val(); /* alert(iVEND);*/ var wvVENDid =  $("#VEND option:selected").text();
        //if(iVEND == "") { alert("Kindly select Vendor"); return false; }
         ...
         // { alert('Debit Amount must balance on table'); return false; }
          ...
          var TRow =   '<tr>'+
                    ...
                    CHSItemDesc+
                     ...
                     '<td class="receiveGL">'+
                      '<input type="hidden" name="DrDesc[]" value="'+DBGLt+'@&@'+DBGL+'@&@'+CHSTotalPrice+'@&@'+BusUnit+'@&@'+EquipC+'@&@'+CHSItemDesc+'@&@'+CHSUnitQty+'@&@'+CHRNum+'@&@'+CHSItem+'@&@CASHR@&@'+CHRID+'@&@'+mItem+'@&@'+mJob+'@&@'+RECEIVER+'@&@'+iVEND+' " />'+
                     DBGLt+
                     '</td>'+
        ...
        var CHSTotalPrice = $('#nTotalPrice').val(); if(CHSTotalPrice == "") { alert("Kindly enter Cash Amount"); return false; }
         ...
         // { alert('Debit Amount must balance on table'); return false; }
          ...
          var TRow =   '<tr>'+
                     ...
                     '<td class="receiveGL">'+
                      '<input type="hidden" name="DrDesc[]" value="'+DBGLt+'@&@'+DBGL+'@&@'+DBNUM+'@&@'+BusUnit+'@&@'+EquipC+'@&@'+CHSItemDesc+'@&@'+CHSUnitQty+'@&@'+CHRNum+'@&@'+CHRItem+'@&@JITEM@&@'+CHRID+'@&@'+mItem+'@&@'+mJob+'@&@'+RECEIVER+'@&@'+iVEND+'" />'+
                     DBGLt+
                     '</td>'+
        ...
        var CHSTotalPrice = $('#nTotalPrice').val(); if(CHSTotalPrice == "") { alert("Kindly enter Cash Amount"); return false; }
         ...
         // { alert('Debit Amount must balance on table'); return false; }
          ...
          var TRow =   '<tr>'+
                     ...
                     '<td class="receiveGL">'+
                      '<input type="hidden" name="CrDesc[]" value="'+DBGLt+'@&@'+DBGL+'@&@'+DBNUM+'@&@'+BusUnit+'@&@'+EquipC+'@&@'+CHSItemDesc+'@&@'+CHSUnitQty+'@&@'+CHRNum+'@&@'+CHRItem+'@&@JITEM@&@'+CHRID+'@&@'+mItem+'@&@'+mJob+'@&@'+RECEIVER+'@&@'+iVEND+'" />'+
                     DBGLt+
                     '</td>'+
                     
                    
                     '<td>0.00</td>'+
                      '<td class="pCredit" amt="'+DBNUM+'">'+addCommas(DBNUM)+'</td>'+
                     '<td><i onclick="$(this).closest(\'tr\').remove(); resetBal();" class="fa fa-trash text-red"></i></td>'+
             '</tr>';
              $('#myModal').modal('hide');
             //$('#EntryChart tbody').prepend(TRow);
     ...
     function resetBal()
     {
      var TB = 0.0; var TC = 0.0;
       ...
       //alert("God is so Loving");
        ...
        $('.pCredit').each(function() {
            var amt = $(this).attr("amt");
            TC = TC + Number(amt);
            //alert(TB)
        });
        
        var finalFigC = addCommas(TC);
       //alert("God is so Loving");
        $('#CRBal').html(finalFigC);
     }
