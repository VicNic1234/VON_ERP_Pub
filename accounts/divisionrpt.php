...
$SuperCredit = 0.0;
$SuperDebit = 0.0;
//First of all Let's loop through the Divisions
 $DivisionQ = mysql_query("SELECT * FROM divisions");
      $NoRowDivisionType = mysql_num_rows($DivisionQ);
      if($NoRowDivisionType > 0) {
          while ($row = mysql_fetch_array($DivisionQ)) 
          {
              $DivisionName = $row['DivisionName'];
              $DivisionID = $row['divid'];
              //$ReadIncome = ReadIncome($DivisionID);
              //$ReadExpense = ReadExpense($DivisionID);
              //$RecChartMaster .= '<tr><td>'.$DivisionName.'</td><td><a href="divisionrpt?FromD='.$FromD.'&ToD='.$ToD.'&TID='.$cidIM.'"><i class="fa fa-eye"></i></a> '.$ReadIncome.'</td><td>'.$ReadExpense.'</td></tr>';
          //////////////////////////////////////////
           $TOTALHEADBAL = 0.0; $TOTALHEADDR = 0.0; $TOTALHEADCR = 0.0;
             ...
             WHERE postings.CRCenter = $DivisionID AND postings.isActive=1
            ...
            {    
               ...
               if($FromD == "" && $ToD == "")
             {
                ...
                $RecChartMaster .= '<tr><td>-</td><td>-</td><td>'.$acid.'</td><td><a href="tbviewacct?FromD='.$FromD.'&ToD='.$ToD.'&TID='.$cidIM.'"><i class="fa fa-eye"></i></a> '.$name.'</td><td>'.$TD.'</td><td><a href="tbviewacct?ACCTID='.$mid2.'"><i class="fa fa-eye text-green"></i></a> '.$mid3.'</td><td>'.$TDescr.'</td><td>'.number_format($DRD,2).'</td><td>'.number_format($CRD,2).'</td>
                <td></td><td></td></tr>'; 
                if($cidIM == 749) {
            ...
            elseif ($FromD != "" && $ToD != "")
            {
                    if ($cid == 3 || $cid == 4)
                    {
                        ...
                        $RecChartMaster .= '<tr><td>-</td><td>-</td><td>'.$acid.'</td><td><a href="tbviewacct?FromD='.$FromD.'&ToD='.$ToD.'&TID='.$cidIM.'"><i class="fa fa-eye"></i></a> '.$name.'</td><td>'.$TD.'</td><td><a href="tbviewacct?ACCTID='.$mid2.'" ><i class="fa fa-eye text-green"></i></a> '.$mid3.' :: '. $GHT.'</td><td>'.$TDescr.'</td><td>'.number_format($DRD,2).'</td><td>'.number_format($CRD,2).'</td>
                        <td></td><td></td></tr>';
                             if($cidIM == 749) {
                             $RETAINEDEARNDR = $DRD; $RETAINEDEARNCR = $CRD;
                            }
          ...
          
          {
                ...
                { 
                   ...
                   $TOTALHEADBALcrn = 0; $TOTALHEADBALdrn = 0; if($TOTALHEADBAL < 0) { $TOTALHEADBALdrn = $TOTALHEADBAL; } else { $TOTALHEADBALcrn = $TOTALHEADBAL; }
                    $RecChartMaster .= '<tr><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: solid"></td><td style="border-top: double; ">'.number_format($TOTALHEADBALdrn,2).'</td><td style="border-top: double; ">'.number_format($TOTALHEADBALcrn,2).'</td><td></td><td></td></tr>';
                }
                //else
                    ...
                    // $RecChartMaster .= '<tr><td></td><td></td><td>'.$acid.'</td><td><a href="viewacct?TID='.$cidIM.'"><i class="fa fa-eye"></i></a> '.$name.'</td><td></td><td></td><td></td><td>'.number_format($TOTALHEADDR,2).'</td><td>'.number_format($TOTALHEADCR,2).'</td></tr>';
                                 if($cidIM == 749) {
                                     $RETAINEDEARNDR = $TOTALHEADDR; $RETAINEDEARNCR = $TOTALHEADCR;
                                      $RecChartMaster .= '<tr><td>'.$acid.'</td><td><a href="tbviewacct?FromD='.$FromD.'&ToD='.$ToD.'&TID='.$cidIM.'"><i class="fa fa-eye"></i></a> '.$name.'</td><td id="RETAINEDEARNDR">'.number_format($TOTALHEADDR,2).'</td><td id="RETAINEDEARNCR">'.number_format($TOTALHEADCR,2).'</td></tr>';
                      
                                    }
                                    else
                                    {
                                     $RecChartMaster .= '<tr><td>'.$acid.'</td><td><a href="tbviewacct?FromD='.$FromD.'&ToD='.$ToD.'&TID='.$cidIM.'"><i class="fa fa-eye"></i></a> '.$name.'</td><td>'.number_format($TOTALHEADDR,2).'</td><td>'.number_format($TOTALHEADCR,2).'</td></tr>';
...
/////////////////////////////////////////////////////////////////////////////////
function ReadIncome($divid)
{
    return 0.0;
}

function ReadExpense($divid)
{
    return 0.0;
}
///////////////////////////////////////////////////////////////////////////////////
...
?>
           ...
           <div class="col-md-12">
              <div class="box">
                <div class="box-header with-border">
                  <h3 class="box-title">Division Report</h3>
              ...
              <div class="form-group col-md-6">
                <span class="btn btn-success pull-right" ><label> 
                     ...
                     <?php if($EXZ == 1) { ?>
                     <input name="EXZ" type="checkbox" checked value="1"  /> Show summation account balance </label> 
                     <?php } else { ?>
                     <input name="EXZ" type="checkbox" value="1"  /> Show summation account balance </label>
                     <?php } ?>
              ...
              <div class="box">
                ...
                <div class="table-responsive">
                   <table id="userTab" class="table table-bordered table-striped">
                      ...
                      <tr>
                       <?php if($EXZ == 1) { ?>
                        <th colspan="9"><center style="font-size:1.5em">DIVISION REPORT</center></th>
                        <? } else { ?>
    ...
    <script type="text/javascript">
     ...
     <script>
         var FG_Retained = '<?php echo $FG_Retained; ?>';
        // alert(FG_Retained);  
              
          var mainRetainedEarningCR = '<?php echo $RETAINEDEARNCR; ?>';
          var mainRetainedEarningDR = '<?php echo $RETAINEDEARNDR; ?>';
          var RTEarnWithGen = (Number(mainRetainedEarningCR) - Number(mainRetainedEarningDR))  + Number(FG_Retained);
         RTEarnWithGen = Number(parseFloat(RTEarnWithGen).toFixed(2)).toLocaleString('en', {
                    minimumFractionDigits: 2
                });
                //alert(RTEarnWithGen);
        $('#RETAINEDEARNCR').html(RTEarnWithGen);
     </script>
