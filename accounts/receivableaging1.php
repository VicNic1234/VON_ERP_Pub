...
//echo $amtOweing = ChkIfOwening(66); exit;
//echo "Paid: " .getTotalAmtPaidToCustomer(66) . "</br> Owe :". ChkIfOwening(66); exit;
...
if ($NoRowLI > 0) {
  while ($row = mysql_fetch_array($resultLI)) 
  {
    ...
    $LastInvoiceDate = LastInvoiceDate($cusid);
    $CusName = $row['CustormerNme'];
    if($amtOweing > 0)
    {
       // $AMTPOSTED =  getTotalAmtPosted($cusid); ////ChkIfOweningLOAD($cusid);//
        $AMTPOSTED = array();
        //if($AMTPOSTED[0] > 0)
        if($amtOweing > 0)
        {
            //$PostedAmt += $AMTPOSTED[0]; 
            //OverRider 
            $AMTPOSTED[0] = $amtOweing;
            $PostedAmt += $AMTPOSTED[0];
            $IVDate = (str_replace("/", "-", $LastInvoiceDate));
            $daysdiffernce = date_diff(date_create($IVDate),date_create($TodayD));
    $DateDiff = $daysdiffernce->format("%R%a");
    
     $TD = $LastInvoiceDate;//$row['TransactionDate'];
     $TDm = DateTime::createFromFormat('Y/m/d', $TD)->format('Y/m/d');
    ////////////////////////////////////////////////////////////////////
    if($FromD == "")
        {
                         if($DateDiff < 31) { $D0T30 += floatval($amtOweing); $AMTPOSTED[1] = $amtOweing; }
                         if($DateDiff > 30 && $DateDiff < 61) { $D31T60 += floatval($amtOweing); $AMTPOSTED[2] = $amtOweing; }
                         if($DateDiff > 60 && $DateDiff < 91) { $D61T90 += floatval($amtOweing); $AMTPOSTED[3] = $amtOweing; }
                         if($DateDiff > 90 ) { $D91UP += floatval($amtOweing); $AMTPOSTED[4] = $amtOweing; }
                        
            //$PostedAmt = $PostedAmt + floatval($amtOweing);
        }
        elseif ($TDm <= $ToD) //$FromD <= $TDm &&
                    ...
                    ////////////////////////////    
            //$D0T30 += $AMTPOSTED[1]; $D31T60 += $AMTPOSTED[2]; $D61T90 += $AMTPOSTED[3]; $D91UP += $AMTPOSTED[4];
             //$D31T60 += $AMTPOSTED[2]; $D61T90 += $AMTPOSTED[3]; $D91UP += $AMTPOSTED[4];
        $AGETAB .= '<tr>
                            <td><a target="_blank" href="cusGLY1?IJK='.$cusid.'">'.$CusName.'</a></td>
...
function ChkIfOweningLOAD($CusID)
{
       $TodayD = date("Y-m-d");
       $FromD = $GLOBALS['FromD']; 
       $ToD = $GLOBALS['ToD'];
       $InvoiceUniqueArray = [];
       $PostedAmt = 0.0; $D0T30 = 0.0; $D31T60= 0.0; $D61T90= 0.0; $D91UP = 0.0;
       $PostedAmt = $TotalPaidAmt = getTotalAmtPaidToCustomer($CusID);
       $BalRemainingToAcount4 = $TotalPaidAmt;
       //OR (GLDescriptionPB =  'Trade Receivable' AND postings1.CusID = '$CusID' AND postings1.isActive=1) 
             
    $TCRD = 0.0; $TDRD = 0.0; $CHKBAL = 0.0;
    $resultChartMaster = mysql_query("SELECT *, postings1.tncid As TNID, enlinvoices.cid As ivcid, reciepts.CheuqeNME As CHKNME  FROM postings1
             LEFT JOIN acc_chart_master ON postings1.GLImpacted = acc_chart_master.mid
             LEFT JOIN enlinvoices ON postings1.ENLINVOICE = enlinvoices.cid
             LEFT JOIN reciepts ON postings1.ChqNo = reciepts.chid
             WHERE (postings1.GLImpacted = '589' AND TransacType = 'DEBIT' AND postings1.CusID = '$CusID' AND postings1.isActive=1 AND postings1.TransactionAmount > 0 )   
             
             
             ORDER BY postings1.TransactionDate");
            ...
            if ($NoRowChartMaster > 0) {
              while ($row = mysql_fetch_array($resultChartMaster)) {
                ...
                //if($TT =="DEBIT")  { $DRD = $TAmount; $TDRD = $TDRD + $TAmount; $CHKBAL = $CHKBAL + $DRD; } else { $DRD = ''; }
                //if($TT =="CREDIT") { $CRD = $TAmount; $TCRD = $TCRD + $TAmount; $CHKBAL = $CHKBAL - $CRD; } else { $CRD = ''; }
                ////////////////////BALANCE OUT DEBIT WITH PAIDED AMOUNT
                $BalRemainingToAcount4 = $BalRemainingToAcount4 - $TAmount;
                 ...
                 $ENLINVOICE = $row['ENLINVOICE'];
                    ...
                    if (in_array($ENLINVOICE, $InvoiceUniqueArray)) 
                     ...
                     //array_push($InvoiceUniqueArray, $ENLINVOICE);
                      ///////////////////////////////////////////////////
                      //$PaidAmt = getTotalAmtPaidOnInvoice($ENLINVOICE);
                    //
                    //$PayBal = floatval($pamt)  - floatval($PaidAmt);
                    //if($PayBal > 0)
                    if($BalRemainingToAcount4 < 0)
                    {
                    $PayBal = abs($BalRemainingToAcount4);
                    $daysdiffernce = date_diff(date_create($IVDate),date_create($TodayD));
                    $DateDiff = $daysdiffernce->format("%R%a");
...
function LastInvoiceDate($CusID)
{
     $resultChartMaster = mysql_query("SELECT * FROM postings1 WHERE (postings1.GLImpacted = '589' AND TransacType='DEBIT' AND postings1.CusID = '$CusID' AND postings1.isActive=1 AND postings1.TransactionAmount > 0 )   
     ORDER BY postings1.TransactionDate");
     $LASTTD = '';
     $NoRowChartMaster = mysql_num_rows($resultChartMaster);
            if ($NoRowChartMaster > 0) {
              while ($row = mysql_fetch_array($resultChartMaster)) {
                $mid = $row['tncid'];
               return  $LASTTD = $row['TransactionDate'];
              }
            }
            else
            {
...
function ChkIfOwening($CusID)
{
       ...
       $InvoiceUniqueArray = [];
       $PostedAmt = 0.0; $D0T30 = 0.0; $D31T60= 0.0; $D61T90= 0.0; $D91UP = 0.0;
       //OR (GLDescriptionPB =  'Trade Receivable' AND postings1.CusID = '$CusID' AND postings1.isActive=1) //AND TransacType = 'DEBIT' 
    $TCRD = 0.0; $TDRD = 0.0; $CHKBAL = 0.0;
    $resultChartMaster = mysql_query("SELECT *, postings1.tncid As TNID, enlinvoices.cid As ivcid, reciepts.CheuqeNME As CHKNME  FROM postings1
             LEFT JOIN acc_chart_master ON postings1.GLImpacted = acc_chart_master.mid
             LEFT JOIN enlinvoices ON postings1.ENLINVOICE = enlinvoices.cid
             LEFT JOIN reciepts ON postings1.ChqNo = reciepts.chid
             WHERE (postings1.GLImpacted = '589' AND postings1.CusID = '$CusID' AND postings1.isActive=1 AND postings1.TransactionAmount > 0 )   
...
function getTotalAmtPaidOnInvoice($enlinvoiceCode)
{
    ////////////////////////PAID ////////////////////////////////
     $FromD = $GLOBALS['FromD']; 
   $ToD = $GLOBALS['ToD'];
 $PaidAmt = 0.0;
   $resultPAID = mysql_query("SELECT * FROM postings1 Where ENLINVOICE='$enlinvoiceCode' AND TransacType='CREDIT' AND GLImpacted='589' AND postings1.isActive=1");
  // $resultPAID = mysql_query("SELECT * FROM postings1 Where CusID='$CusID' AND TransacType='CREDIT' AND GLImpacted='589'");
...
function getTotalAmtPaidToCustomer($CusID)
{
    ////////////////////////PAID ////////////////////////////////
     $FromD = $GLOBALS['FromD']; 
   $ToD = $GLOBALS['ToD'];
...
function getTotalAmtPosted($CusID)
{
   ...
   $InvoiceUniqueArray = [];
   $PostedAmt = 0.0; $D0T30 = 0.0; $D31T60= 0.0; $D61T90= 0.0; $D91UP = 0.0;
   /*$resultPOSTING = mysql_query("SELECT * FROM postings1 
   LEFT JOIN enlinvoices ON postings1.ENLINVOICE = enlinvoices.cid
   Where ENLINVOICE='$enlinvoiceCode' AND TransacType='DEBIT' AND GLImpacted='589'");*/
   $resultPOSTING = mysql_query("SELECT * FROM postings1 
   LEFT JOIN enlinvoices ON postings1.ENLINVOICE = enlinvoices.cid
   Where postings1.CusID='$CusID' AND postings1.TransacType='DEBIT' AND postings1.GLImpacted='589' AND postings1.isActive=1");
