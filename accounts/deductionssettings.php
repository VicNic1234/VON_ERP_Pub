...
//Let's Read Earnings
$RecAccSet = "";
$resultAccSet = mysql_query("SELECT *, deductions_settings.id AS EnrsID,
  deductions_settings.Description AS ErnDes,
  opt_cal_method.Description AS CalMedDes,
  opt_employee_group.Description AS EmpGrp,
  acc_chart_master.account_name AS GLNME,
  deductions_settings.isActive As EnrsStatus FROM deductions_settings
  INNER JOIN acc_chart_master ON deductions_settings.GLMaster = acc_chart_master.mid
  INNER JOIN opt_cal_method ON deductions_settings.CalMethod = opt_cal_method.calid
  INNER JOIN opt_earning_freq ON deductions_settings.EarningFrequ = opt_earning_freq.enfreqid 
  INNER JOIN opt_employee_group ON deductions_settings.AppliedToStaffGroup = opt_employee_group.empgid 
  ");
...
function addAcc(elem)
{
    ...
    var size='standart';
            ...
            '<div class="col-md-12">' +
            '<form role="form" action="addDeduction" method="POST" ><div class="form-group">' +
              '<div class="form-group col-md-6"><label>Description: </label><input type="text" class="form-control" name="newDeduction" placeholder="Enter Deduction Description"></div>' +
              ...
              '<div class="form-group col-md-6"><label>Deduction Frequency: </label><select class="form-control" name="newEarnFreq">'+ OptEarningFrequ +'</select></div>' +
              ...
              '<button type="submit" class="btn btn-primary pull-right">Add Deduction</button><br/></form>'+
              '</div></div>';
            var title = 'New Deduction';
            var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>';
           ...
           <div class="col-md-12">
              ...
              <div class="box">
                <div class="box-header">
                  <h4> Deductions Settings </h4>
                 <button class="btn btn-success pull-right" id="addAcct" onclick="addAcc(this)" > Add Deductions</button>
                </div><!-- /.box-header -->
                <div class="box-body">
