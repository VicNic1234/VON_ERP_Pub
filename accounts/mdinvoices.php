...
*/
 ...
 vendorsinvoices.IVNo AS IVNo FROM vendorsinvoices 
 LEFT JOIN suppliers ON vendorsinvoices.VendSource = suppliers.supid
 LEFT JOIN users ON vendorsinvoices.RaisedBy = users.uid
 
  WHERE vendorsinvoices.isActive=1 AND (vendorsinvoices.COOAppDate<>'' OR vendorsinvoices.MDOfficeOn<>'') AND Paid <> 3 
 "); //WHERE isActive=1 ORDER BY cid //GROUP BY PONUM //GROUP BY IVNo ORDER BY vendorsinvoices.cid
...
//fetch tha data from the database
	 if ($NoRowLI > 0) 
   {
    ...
    $DDState = ""; $GMCSState = ""; $MDState = "";

    $GMCSAppComment = $row['GMCSAppComment']; 
    $GMCSAppDate = $row['GMCSAppDate'];
    
    $MDOfficer = $row['MDOfficer']; $MDOfficerComment = $row['MDOfficeComment']; 
    $MDOfficerOn = $row['MDOfficerOn'];
    ...
    if($MDOfficerOn != "") { $MDState = '<i class="fa fa-legal text-green"></i>';  }
    elseif ($MDOfficerComment != "") { $MDState = '<i class="fa fa-comment text-green"></i>';  }
    else { $MDState = '<i class="fa fa-edit text-red"></i>'; }
    

    if($FileLink != ""){
    $FileLinkn = '<a target="_blank" title="Download Invoice document" href="'.$FileLink.'"><i class="fa fa-link"></i></a>';
    }
    else
    {
      $FileLinkn = '';
    ...
    $AccountState = '<a href="paidInv?id='.$cid.'"><button class="btn btn-success" > Paid </button></a>';
    
    $ViewCon = '<a title="View full Vendor\'s Invoice" href="viewmdinvoice?poid='.$cid.'"><i class="fa fa-edit"></i></a>';
    $PrintPO = '<form action="printMDINVOICE" method="POST">
