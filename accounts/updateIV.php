...
$UID = $_SESSION['uid'];
  ...
  $DBDGL = trim(strip_tags($_POST['DBDGL']));
  ...
  if($VenInvoiceID == "")
  {
         $_SESSION['ErrMsg'] = "Kindly enter your Invoice Number";
  if (isset($_SERVER["HTTP_REFERER"])) {
        header("Location: " . $_SERVER["HTTP_REFERER"]);
    }
     exit;
  }
  //we need to know if its a duplicate Invoice
  
  //First we need to check if the Invoice already exist
 $resultIN= mysql_query("SELECT * FROM acct_vendorsinvoices WHERE IVNo='".$VenInvoiceID."' AND cid <> '".$ConID."'");
//check if user exist
...
if($FILEURL != "")
{
  $query1 = "UPDATE acct_vendorsinvoices SET NGNRate='".$NGNRate."', CRDGL='".$DBDGL."', BnkID='".$ENLBANK."', ScopeOfSW='".$Scope."', IVNo='".$VenInvoiceID."', Comment='".$Comment."', conDiv='".$conDiv."', IVDate='".$IVDate."', ENLATTN='".$ENLATTN."', VENATTN='".$VENATTN."', VendSource='".$VendSource."', FileLink='".$FILEURL."', Currency='".$currency."', PDFNUM='".$PDFNum."', PONUM='".$PONum."' WHERE cid='".$ConID."'";
}
else
{
  $query1 = "UPDATE acct_vendorsinvoices SET NGNRate='".$NGNRate."', CRDGL='".$DBDGL."', BnkID='".$ENLBANK."', ScopeOfSW='".$Scope."', IVNo='".$VenInvoiceID."', Comment='".$Comment."', conDiv='".$conDiv."', IVDate='".$IVDate."', ENLATTN='".$ENLATTN."', VENATTN='".$VENATTN."', VendSource='".$VendSource."', Currency='".$currency."', PDFNUM='".$PDFNum."', PONUM='".$PONum."' WHERE cid='".$ConID."'";
}
