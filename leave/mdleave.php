...
$resultLeave = mysql_query("SELECT *, empleave.id AS leaveid FROM empleave 
  ...
  WHERE (divisions.GM = '$StaffID' OR divisions.DH = '$StaffID' OR empleave.Status = 6) AND isActive='1'"); 
//check if user exist
	...
	while ($row = mysql_fetch_array($resultLeave)) {
     ...
     $Approved = $row['Status'];
      ...
      else if ($Approved == 6)
      {
        ...
        $Approved = '<a onclick="BackToHR(\'MD\', \''.$reqid.'\')"><button title="By Clicking, you send back to HR" class="btn btn-warning"><i class="fa fa-undo"></i> Send To Back HR</button></a> &nbsp; <a onclick="ApproveByMD(\'MD\', \''.$reqid.'\')" ><button title="By Clicking, you Approve this leave" class="btn btn-success"><i class="fa fa-send"></i> Approved By MD</button></a>';
...
?>
    ...
    <script type="text/javascript">
     ...
     <script type="text/javascript">
       function ApproveByMD(actor, leaveID)
      {
        
          var size='standart';
                  var content = '<form role="form" action="ApproveByMD" method="POST" ><div class="form-group">' +
