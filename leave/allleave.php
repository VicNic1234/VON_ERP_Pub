...
$resultLeave = mysql_query("SELECT *, empleave.id AS leaveid FROM empleave 
  WHERE isActive='1' ORDER BY empleave.id DESC"); 
//check if user exist
...
if ($NoRow > 0) 
{
	...
	while ($row = mysql_fetch_array($resultLeave)) {
     ...
     $SN = $SN + 1;
      ...
      else if ($Approved == 5)
      {
        ...
        /* $Approved = '
         ...
         <a onclick="SendToGMCS(\'HR\', \''.$reqid.'\')"><button title="By Clicking, you send to GM Coporate Services" class="btn btn-success"><i class="fa fa-send"></i> Send to GM of CS</button></a>
          &nbsp; 
         <a onclick="CancelLeave(\'HR\', \''.$reqid.'\')"><button title="By Clicking, you are cancelling this leave" class="btn btn-danger"><i class="fa fa-trash"></i> Cancel</button></a>
         ';
         */
      }
      else if ($Approved == 6)
      ...
      {
        $Approved = "<b style='color:green'>Approved</b>";
      }
      else if($Approved == 8)
      {
        $Approved = "<b style='color:red'>Cancelled</b>";
      }
       else if($Approved == 9)
      {
	    ...
	    $Record .='
            ...
            <td>'.$NoOfDays.'</td>
            <td>'.$Approved.'</td>
             <td><a target="_blank" href="leavereroute?rNo='.$reqid.'" ><i class="fa fa-eye"></i></a></td>
