...
$resultLeave = mysql_query("SELECT *, empleave.id AS leaveid FROM empleave 
  ...
  WHERE divisions.DH = '$StaffID' AND isActive='1'"); 
//check if user exist
...
if ($NoRow > 0) 
{
	...
	while ($row = mysql_fetch_array($resultLeave)) {
     ...
     $Approved = $row['Status'];
      ...
      else if ($Approved == 2) //href="BackToHDept?id='.$reqid.'"
      {
        //$Approved = "Pending with Head of Division"; //href="SendToDivGM?id='.$reqid.'
        $Approved = '<a onclick="BackToHDept(\'DivHead\', \''.$reqid.'\')"><button title="By Clicking, you send back to Head of Department" class="btn btn-warning"><i class="fa fa-undo"></i> Send To Head of Dept</button></a> &nbsp; <a onclick="SendToDivGM(\'DivHead\', \''.$reqid.'\')"><button title="By Clicking, you send to GM of Division" class="btn btn-success"><i class="fa fa-send"></i> Send to GM of Division</button></a>';
      }
      else if ($Approved == 3)
      {
...
?>
	...
	<script type="text/javascript">
     ...
     <script type="text/javascript">
       function SendToDivGM(actor, leaveID)
      {
        
          var size='standart';
                  var content = '<form role="form" action="SendToDivGM" method="POST" ><div class="form-group">' +
