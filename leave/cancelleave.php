...
$sql_leave=mysql_query("SELECT * FROM empleave LEFT JOIN users ON empleave.uid = users.uid WHERE empleave.id='$reqID'");

$isLeave = mysql_num_rows($sql_leave);
if($isLeave > 0 ) { 
	while ($row = mysql_fetch_array($sql_leave)) {
	    $Requester = $row['uid'];
	    $RequesterFN = $row['Firstname'] . ' ' . $row['Surname'];
...
if($HRAppComment != "") { $ReqMSG = '<div class="rcorners1">'.$HRAppComment.'  | <b>'.$HRAppDate.'</b></div> <br/> '. $ReqMSG; }
//We need to know if this person is a DeptHead or DivHead or GM
$resultLeave = mysql_query("UPDATE empleave SET HRApp='$StaffID', HRAppDate='$TodayD', HRAppComment='$ReqMSG', Status='8' WHERE id = '".$reqID."'");
		require_approval_action_notification ($HODEmail, $HODFullname, $reqID, "Requester"); 
...
//Lets sned the EMail here, No time
		function require_approval_action_notification ($email, $fullnme, $pdfid, $role)
		{
		        ...
		        $MsgTitle = "Elshcon ERP [Leave has been cancelled]";
		        $GLOBALS['mail']->Subject = $MsgTitle;
		        $GLOBALS['mail']->AddAddress($email, $fullnme);
		       

		        $msgBody = 'Kindly check you leave Request on Elshcon ERP.<br/><br/><br/>';
