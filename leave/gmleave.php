...
if($StaffID == "")
{
 ...
 function getUserinfo($uid)
     ...
     function getDeptinfo($uid)
     {
        ...
        while ($row = mysql_fetch_array($resultUserInfo)) 
        {
             return $DepatID = $row['DeptID'];
        }
     }

$resultLeave = mysql_query("SELECT *, empleave.id AS leaveid, department.hod as HOD FROM empleave 
  LEFT JOIN department ON  empleave.Dept = department.id
  LEFT JOIN divisions ON  department.DivID = divisions.divid
  WHERE divisions.GM = '$StaffID' AND isActive='1'"); 
//check if user exist
...
if ($NoRow > 0) 
{
	...
	while ($row = mysql_fetch_array($resultLeave)) {
     ...
     $RequesterDept = getDeptinfo($row['uid']); 
     
     ///////////////////////////
     /*$resultDept = mysql_query("SELECT *, divisions.DH As DHa, divisions.GMa AS GM department WHERE id='$RequesterDept' LEFT JOIN divisions ON department.DivID = divisions.divid"); 
     $NfoRow = mysql_num_rows($resultDept);
     	while ($bbrow = mysql_fetch_array($resultDept)) {
     	     $ReDH =     $bbrow['DHa'];
     	 echo   $ReGM =     $bbrow['GMa'];
   ...
   // exit;
     ...
     $Approved = $row['Status'];
      ...
      else if ($Approved == 1 && ($HODID ==  $StaffID || $HODID == 0) && $DHID != $StaffID )
      {
      ...
      else if ($Approved == 1 && ($HODID ==  $StaffID || $HODID == 0) && $DHID == $StaffID && $GMID != $StaffID  )
      {
          $Approved = '<a onclick="BackToRequester(\'HOD\', \''.$reqid.'\')"><button title="By Clicking, you send back to Requester" class="btn btn-warning"><i class="fa fa-undo"></i> Send To Requester</button></a> &nbsp; <a onclick="SendToDivH(\'HOD\', \''.$reqid.'\')" ><button title="By Clicking, you send to GM of Division" class="btn btn-success"><i class="fa fa-send"></i> Send to GM of Division</button></a>';
      }
      else if ($Approved == 1 && ($HODID ==  $StaffID || $HODID == 0) && $DHID == $StaffID && $GMID == $StaffID)
      {
      ...
      else if ($Approved == 2 || $StaffID == $DHID)
      {
        //$Approved = "Pending with Head of Division";
        $Approved = '<a onclick="BackToHDept(\'GM\', \''.$reqid.'\')"><button title="By Clicking, you send back to Head of Department" class="btn btn-warning"><i class="fa fa-undo"></i> Send To Head of Department </button></a> &nbsp; <a onclick="SendToHR(\'GM\', \''.$reqid.'\')"><button title="By Clicking, you send to HR" class="btn btn-success"><i class="fa fa-send"></i> Send to HR</button></a>';
...
?>
    ...
    <script type="text/javascript">
     ...
     <script type="text/javascript">
       function SendToHR(actor, leaveID)
      {
        
          var size='standart';
                  var content = '<form role="form" action="SendToHR" method="POST" ><div class="form-group">' +
      ...
      function BackToHDiv(actor, leaveID)
      {
        
          var size='standart';
                  var content = '<form role="form" action="BackToHDiv" method="POST" ><div class="form-group">' +
                   '<input type="hidden" class="form-control"  name="ReqCode" value="'+ leaveID +'">' +
                    '<input type="hidden" name="actor" value="'+actor+'" />'+
       ...
       function BackToHDept(actor, leaveID)
      {
        
          var size='standart';
                  var content = '<form role="form" action="BackToHDept" method="POST" ><div class="form-group">' +
                   '<input type="hidden" class="form-control"  name="ReqCode" value="'+ leaveID +'">' +
                    '<input type="hidden" name="actor" value="'+actor+'" />'+
                    '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Message to HOD: </label><input type="text" class="form-control" id="eaccCode" name="ReqMSG" placeholder=""  value="Kindly help expedite. Thanks" required ></div>' +
