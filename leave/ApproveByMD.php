...
$sql_hr=mysql_query("SELECT * FROM empleave LEFT JOIN users ON empleave.uid = users.uid WHERE id='$reqID'");
...
if($MDAppComment != "") { $ReqMSG = '<div class="rcorners1">'.$MDAppComment.'  | <b>'.$MDAppDate.'</b></div> <br/> '. $ReqMSG; }
//We need to know if this person is a DeptHead or DivHead or GM
$resultLeave = mysql_query("UPDATE empleave SET MDApp='$StaffID', MDAppDate='$TodayD', MDAppComment='$ReqMSG', Status='7' WHERE id = '".$reqID."'");
		require_approval_action_notification ($HODHRID, $HODHRFullname, $reqID, "Requester"); 
...
//Lets sned the EMail here, No time
		function require_approval_action_notification ($email, $fullnme, $pdfid, $role)
		{
		        ...
		        $MsgTitle = "Elshcon ERP [Approved by MD Leave]";
		        ...
		        $msgLink = 'https://www.elshcon.space/leave/myleave';
