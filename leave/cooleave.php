...
$resultLeave = mysql_query("SELECT *, empleave.id AS leaveid FROM empleave 
  ...
  WHERE (divisions.GM = '$StaffID' OR divisions.DH = '$StaffID' OR empleave.Status = 9) AND isActive='1'"); 
//check if user exist
	...
	while ($row = mysql_fetch_array($resultLeave)) {
     ...
     $Approved = $row['Status'];
      ...
      else if ($Approved == 2)
      {
        ...
        if($DivHApp == $StaffID)
        {
         $Approved = '<a onclick="BackToHR(\'GMCS\', \''.$reqid.'\')"><button title="By Clicking, you send back to HR" class="btn btn-warning"><i class="fa fa-undo"></i> Send To Back HR</button></a> &nbsp; <a onclick="ApproveByGMCS(\'GMCS\', \''.$reqid.'\')" ><button title="By Clicking, you Approve this leave" class="btn btn-success"><i class="fa fa-send"></i> Approved By GM CS</button></a>';
        }
        else
          { 
      ...
      else if ($Approved == 4)
      {
        ...
        $Approved = '<a onclick="BackToHR(\'GMCS\', \''.$reqid.'\')"><button title="By Clicking, you send back to HR" class="btn btn-warning"><i class="fa fa-undo"></i> Send To Back HR</button></a> &nbsp; <a onclick="SendToMD(\'GMCS\', \''.$reqid.'\')" ><button title="By Clicking, you Approve this leave" class="btn btn-success"><i class="fa fa-send"></i> Send to MD</button></a>';
      }
      else if ($Approved == 5)
      {
        ...
        else if($Approved == 9)
      {
       // $Approved = "<b style='color:orange'>Pending with COO</b>";
         $Approved = '<a onclick="BackToHR(\'COO\', \''.$reqid.'\')"><button title="By Clicking, you send back to HR" class="btn btn-warning"><i class="fa fa-undo"></i> Send To Back HR</button></a> &nbsp; ';
         $Approved .= '<a onclick="SendToMD(\'COO\', \''.$reqid.'\')"><button title="By Clicking, you send to MD" class="btn btn-success"><i class="fa fa-send"></i> Send To MD</button></a>  &nbsp;';
...
?>
    ...
    <script type="text/javascript">
     ...
     <script type="text/javascript">
       function ApproveByGMCS(actor, leaveID)
      {
        
          var size='standart';
                  var content = '<form role="form" action="ApproveByGMCS" method="POST" ><div class="form-group">' +
                   '<input type="hidden" class="form-control"  name="ReqCode" value="'+ leaveID +'">' +
                    '<input type="hidden" name="actor" value="'+actor+'" />'+
                    '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Approval Message: </label><input type="text" class="form-control" id="eaccCode" name="ReqMSG" placeholder=""  value="Kindly help expedite. Thanks" required ></div>' +

                   '<button type="submit" class="btn btn-success pull-right">Send</button><br/></form>';
                  var title = 'Approved GM Coporate Services';
      ...
      function BackToHR(actor, leaveID)
      {
        
          var size='standart';
                  var content = '<form role="form" action="BackToHR" method="POST" ><div class="form-group">' +
                   '<input type="hidden" class="form-control"  name="ReqCode" value="'+ leaveID +'">' +
                    '<input type="hidden" name="actor" value="'+actor+'" />'+
                    '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Message to HR: </label><input type="text" class="form-control" id="eaccCode" name="ReqMSG" placeholder=""  value="Kindly help expedite. Thanks" required ></div>' +

                   '<button type="submit" class="btn btn-success pull-right">Send</button><br/></form>';
                  var title = 'Send Back To HR';
                  var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>';
        ...
        function SendToMD(actor, leaveID)
      {
          ...
          var size='standart';
                  var content = '<form role="form" action="SendToMDCOO" method="POST" ><div class="form-group">' +
                   '<input type="hidden" class="form-control"  name="ReqCode" value="'+ leaveID +'">' +
                    '<input type="hidden" name="actor" value="'+actor+'" />'+
