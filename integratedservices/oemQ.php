...
<script language="javascript">
        ...
        function open_container(LID, rfq, mst, qty, uom, lides, uw, uc, ds, fp, hsc, hst, pp, ip)
        {
			...
			'<div class="form-group" style="width:60%; display:inline-block; margin:12px;"><label>Local Cost: </label><input type="text" class="form-control" id="LocCos" name="LocCos" placeholder="Local Cost" readonly ></div>' +
			
			'<div class="form-group" style="width:80%; margin:12px;"><label>DDP Price (CIF + Local Cost): </label><input type="text" class="form-control" id="DPPPrice" name="DPPPrice" placeholder="DDP Price" readonly ></div>' +
...
//fetch tha data from the database
	...
	while ($row = mysql_fetch_array($resultLI)) {
	  ...
	  $Statu = $row ['Status'];
	  if ($Statu == "OPEN") {
	   $SStatus = '<i class="fa fa-cog" r="'.$LitID.'" id="hj'.$LitID.'" onclick="orfq(this);"></i>';
	  }
	  else
	  {
	  $SStatus = '<i class="fa fa-check" r="'.$LitID.'" id="hj'.$LitID.'" onclick="crfq(this);"></i>';
	  }
	    ...
	    $Record .='
					 <tr>
						...
						<td><a '.  'onclick="open_container('.$LitID.',\''.$RFQn.'\',\''.$MatSer.'\',\''.$Qty.'\',\''.$UOM.'\',\''.$Description.'\',\''.$UnitWeight.'\',\''.$UnitCost.'\',\''.$Discount.'\',\''.$FreightPercent.'\',\''.$HScode.'\',\''.$HsTarrif.'\',\''.$PackingPercent.'\',\''.$InsurePercent.'\');">'. '<span class="glyphicon glyphicon-edit"></span></a></td>
						<td>'.$SStatus.'</td>
