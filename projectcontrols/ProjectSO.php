     ...
     if ($_POST['searchso'])
        {
          ...
          if($fmCusSO == "All")
          {
            ...
            ON so.SOAssignStaff=users.uid WHERE (STR_TO_DATE(SOdate, '%m/%d/%Y') between '$fmfrmDate' and '$fmtoDate') AND SOType=2 Order By poID DESC");
          }
          else
          {
            $resultRFQ= mysql_query("SELECT * FROM so LEFT JOIN users 
            ON so.SOAssignStaff=users.uid WHERE Customer='$fmCusSO' AND (STR_TO_DATE(SOdate, '%m/%d/%Y') between '$fmfrmDate' and '$fmtoDate') AND SOType=2 Order By poID DESC");
          }
//exit;
        ...
        {
         $resultRFQ= mysql_query("SELECT * FROM so LEFT JOIN users 
         ON so.SOAssignStaff=users.uid WHERE Status='0' AND SOType=2 Order By poID DESC");
        }
//check if user exist
    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
         ...
         &nbsp; &nbsp; &nbsp;
            <a href="./"><span>Back <i class="fa fa-backward"></i></span></a>
          </h1>
