...
//check if user exist
 ...
 if ($_GET['sRFQ'] == "")
 {
 $resultLI = mysql_query("SELECT * FROM polineitems WHERE ProjectControl='1' ORDER BY LitID DESC LIMIT 10");
//check if user exist
 $NoRowLI = mysql_num_rows($resultLI);
 }
 else if ($_GET['sRFQ'] == "ALL")
 {
 $resultLI = mysql_query("SELECT * FROM polineitems WHERE ProjectControl='1' ORDER BY LitID DESC");
//check if user exist
 $NoRowLI = mysql_num_rows($resultLI);
 }
 else
 {
 $resultLI = mysql_query("SELECT * FROM polineitems WHERE RFQCode='".$_GET['sRFQ']."' AND ProjectControl='1' ORDER BY LitID DESC");
  ...
  function chngmkup()
   ...
   var DPPprice = LocCost + CIF;
    ...
    document.getElementById('CIFPPrice').value = CIFPPrice.toFixed(2);
        ...
        {
			var title = 'Prepare line item for SO, LineItemID : '+LID + ' in RFQ No. : '+rfq;
...
//fetch tha data from the database
    ...
    $MOS = $row ['MOS'];
     ...
     {
    // $PCStatus = '<i class="fa fa-cog" title="Open for Project Control" r="'.$LitID.'" jm="hpc'.$LitID.'" id="hpc'.$LitID.'" onclick="cpc1(this);"></i>';
     $PClegal = '<input type="checkbox" title="Legal Terms not acertained" r="'.$LitID.'" jm="hpc'.$LitID.'" id="hplg'.$LitID.'" onclick="cpcLegal(this);"></input>';
     }
     if ($legal == '1') 
     {
     //$PCStatus = '<i class="fa fa-check" title="Close from Project Control" r="'.$LitID.'" id="hpc'.$LitID.'" onclick="cpc(this);"></i>';
     $PClegal = '<input  type="checkbox" checked title="Legal Terms is Ok" cm="CreateLegal" r="'.$LitID.'" jm="hpc'.$LitID.'" id="hplg'.$LitID.'" onclick="cpcLegal(this);"></input>';
     }
