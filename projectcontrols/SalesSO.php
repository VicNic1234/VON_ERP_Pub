     ...
     if ($_POST['searchso'])
        {
          ...
          if($fmCusSO == "All")
          {
            ...
            ON so.SOAssignStaff=users.uid WHERE (STR_TO_DATE(SOdate, '%m/%d/%Y') between '$fmfrmDate' and '$fmtoDate') AND SOType=1 Order By poID DESC");
          }
          else
          {
            $resultRFQ= mysql_query("SELECT * FROM so LEFT JOIN users 
            ON so.SOAssignStaff=users.uid WHERE Customer='$fmCusSO' AND (STR_TO_DATE(SOdate, '%m/%d/%Y') between '$fmfrmDate' and '$fmtoDate') AND SOType=1 Order By poID DESC");
          }
//exit;
        ...
        {
         $resultRFQ= mysql_query("SELECT * FROM so LEFT JOIN users 
         ON so.SOAssignStaff=users.uid WHERE Status='0' AND SOType=1 Order By poID DESC");
        }
//check if user exist
