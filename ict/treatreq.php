...
$RecChartClass = "";
$resultChartClass = mysql_query("SELECT * FROM ictreq 
  LEFT JOIN department ON ictreq.Deparment = department.id
  LEFT JOIN reqcategory ON ictreq.Purpose = reqcategory.id
   "); 
//LEFT JOIN users ON ictreq.staffID = users.uid
$NoRowChartClass = mysql_num_rows($resultChartClass);
if ($NoRowChartClass > 0) {
  while ($row = mysql_fetch_array($resultChartClass)) {
    $itreq = $row['reqid'];
    $staffName = $row['staffName']; //
    $ActorComment = $row['ActorComment']; //
    $RequestID = $row['RequestID'];
    $RequestDate = $row['RequestDate'];
    $ItemDes = $row['ItemDes']; //
    $DeptmentName = $row['DeptCode']; //
    ...
    <td>'.$ActorComment.'</td>
    <td>'.$Status.'</td>
    <td><a acctnme="'.$RequestID.'" acctsta="'.$ActorComment.'" acctid="'.$itreq.'"  onclick="editAcc(this)"><i class="fa fa-edit"></i></a></td>
...
function editAcc(elem)
{
 ...
 var acctsta = $(elem).attr("acctsta"); 
 //var ShortCode = $(elem).attr("ShortCode"); 
    var size='standart';
            var content = '<form role="form" action="treatcase" method="POST" ><div class="form-group">' +
             '<input type="hidden" value="'+acctid+'" id="idClass" name="idClass" />'+

             '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>RequestID: </label><input type="text" class="form-control" name="RequestID" value="'+acctnme+'" placeholder="" required readonly ></div>' +

              '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Comment: </label><input type="text" class="form-control" id="nmeClass" name="CaseComment" value="'+acctsta+'" placeholder="" required ></div>' +

              '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label> Open: <input type="radio" name="rstate" value="Open" checked ></label> <br/> <label> Closed: <input type="radio" name="rstate" value="Closed" ></label> <br/>  <label> Cancelled: <input type="radio" name="rstate" value="Cancelled" ></label></div>' +

              
             
             '<button type="submit" class="btn btn-primary pull-right">Update</button><br/></form>';
            var title = 'Treat Request';
