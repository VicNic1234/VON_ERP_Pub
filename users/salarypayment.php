     ...
     $PORAppStatus = '<input  type="checkbox" checked title="Select to Apply Action" id="hpc'.$uid.'" r="'.$uid.'" onclick="cpc(this);"></input>';
      ...
      $Record .='
           <tr>
            ...
            <td><a '.  'onclick="open_container('.$uid.',\''.$SurName.'\',\''.$Firstname.'\',\''.$BankName.'\',\''.$BankAccount.'\',\''.$BankAccountType.'\',\''.$salary.'\');">'.  '<span class="glyphicon glyphicon-edit"></span></a></td>
            <td>'.$PORAppStatus.'</td>
...
<br>
	  ...
	  <script language="javascript">
        ...
        {  
			...
			'<select class="form-control" id="BankAccountType" name="BankAccountType" placeholder="BankAccountType" required>' +
            ...
            'Account Number: <input type="text" class="form-control" id="AccountNumber" name="AccountNumber" placeholder="Account Number" value="'+bnkacct+'" required />' +
            
            '</div>' +
		    '<div class="form-group has-feedback" style="width:40%; display: inline-block; margin:12px;">' +
            'Bank Name: <input type="text" class="form-control" id="BankName" name="BankName" placeholder="Bank Name" value="'+bnk+'" required />' +
           
            '</div>' +
             '<div class="form-group has-feedback" style="width:40%; display: inline-block;">' +
            'Salary: <input type="number" class="form-control" id="salary" readonly name="salary" placeholder="Salary" value="'+salary+'" required />' +
           
            '</div>' +
			'</div>'+
			   
			'<button type="button" onclick="alert(\' Kindly setup your payment aggregator API \')" class="btn btn-success pull-right">Pay</button></form>';
