...
$ModeOfEmp = trim(strip_tags($_POST['MOE']));
$BusUnt = trim(strip_tags($_POST['BusUnt']));
$jbtitle = trim(strip_tags($_POST['jbtitle']));
$posti = trim(strip_tags($_POST['posti']));
$rptmgr = trim(strip_tags($_POST['rptmgr']));
$empsta = trim(strip_tags($_POST['empsta']));
$DOJ = mysql_real_escape_string(trim(strip_tags($_POST['DOJ'])));
$YOEx = trim(strip_tags($_POST['YOEx']));
$WkEx = trim(strip_tags($_POST['WkEx']));
$WkPhn = trim(strip_tags($_POST['WkPhn']));
...
ModeOfEmp, BusinessUnit, JobTitle, JobPosition, RptMgr, EmployeeStatus, DateOfJoining, YearsOfExp, WorkExt, WorkPhone) 
VALUES ('$staffid','$dept','$fpwsd','$staffrole','$FirstName','$SurName','$Gender','$staffmail','$staffphn','$DOB','$data',
'$ModeOfEmp','$BusUnt','$jbtitle','$posti','$rptmgr','$empsta','$DOJ','$YOEx','$WkEx','$WkPhn');";
