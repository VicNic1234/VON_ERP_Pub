...
$sqlPOREQ=mysql_query("SELECT * FROM cashreq WHERE RequestID='$ReqCode'");
while ($rowPOREQ = mysql_fetch_array($sqlPOREQ)) {
       $MDAppDate = $rowPOREQ['COOAppDate'];  $MDComment = $rowPOREQ['COOComment'];
     }
     
     if($MDComment != "")
     {
       $Mgs =  '<div class="rcorners1">'.mysql_real_escape_string($MDComment).'  | <b>'.$MDAppDate.'</b></div> <br/> '. $Mgs;
     }
...
//FOR APPROV
   mysql_query("UPDATE cashreq SET Approved='".$Apn."', LastActor='COO\'s Office', Status='".$ReqMSG."', COOApp='".$uid."', ApprovedAmount='".$AAMT."', ApprovedBy='".$staffname."', COOAppDate='".$TodayD."',  COOComment='".$Mgs."' WHERE RequestID = '$ReqCode'");
...
function require_approval_action_notification ($email, $fullnme, $pdfid, $role, $msg)
{
        ...
        $MsgTitle = "Elshcon ERP [MD's Office]";
        $GLOBALS['mail']->Subject = $MsgTitle;
        $GLOBALS['mail']->AddAddress($email, $fullnme);
       // $GLOBALS['mail']->AddCC("henry.oshikoya@elshcon.com", "HENRY AYODELE");
     

        $msgBody = 'Kindly click on link below to view request on Elshcon ERP which have been Treated By COO\'s Office,<br/><br/> with message : <br/><br/><em>'.$msg.'</em>';
...
///////////////////////////////////////
$_SESSION['ErrMsgB'] = "Approved By MD's Office,  ".$REQFullname." have been notified. Thanks";
header('Location: cashmdcoo?pdfid='.$ReqCode);
