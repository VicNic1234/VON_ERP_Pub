...
//Let's Get All GL accounts
 ...
 while ($row = mysql_fetch_array($GLAccounts)) {
     ...
     $acctvariable = $row['account_code'];
     $GLmid = $row['mid'];
      $GLAcct = str_replace("'", "", $row['account_name']);
      $GLAcct = str_replace("/", "-", $GLAcct);
      $GLAcct = str_replace(".", "-", $GLAcct);
...
function printDiv(divName) {
   ...
   <span style="float:right" onclick="POSTPETTY()" class="btn btn-warning">Processing Payment</span>
   <?php } ?>
