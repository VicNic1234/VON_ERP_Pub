...
////////////////////////////////////////////////////////////////////////////
    ...
    $sql_res=mysql_query("SELECT * FROM users WHERE COO='1'"); //DeptID = '$DeptID' AND 

 $isHOD = mysql_num_rows($sql_res); 
if($isHOD < 1) { 
    $_SESSION['ErrMsg'] = "No COO configured, contact ICT. Thanks";
     if (isset($_SERVER["HTTP_REFERER"])) {
        header("Location: " . $_SERVER["HTTP_REFERER"]);
    }
    exit; 
...
//Update the rpo table
    mysql_query("UPDATE cashreq SET Approved='18', LastActor='Internal Control', Status='Approved', COOApp='".$MDID."', DDApp='".$uid."', ApprovedBy='".$staffname."', DDAppDate='".$TodayD."',  DDAppComment='".$hodMSG."' WHERE RequestID = '$ReqCode'");
