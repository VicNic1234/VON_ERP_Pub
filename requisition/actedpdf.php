...
<?php
 ...
 LEFT JOIN users ON poreq.staffID = users.uid where poreq.isActive=1 AND ((poreq.staffID = $ACTNOWID)
 OR (poreq.UserApp = $ACTNOWID)
 OR (poreq.SupervisorApp = $ACTNOWID)
 OR (poreq.DeptHeadApp = $ACTNOWID)
 OR (poreq.DivHeadApp = $ACTNOWID)
 OR (poreq.CPApp = $ACTNOWID)
 OR (poreq.CSApp = $ACTNOWID)
 OR (poreq.MgrApp = $ACTNOWID)
 OR (poreq.DDApp = $ACTNOWID)
 OR (poreq.DDOfficerApp = $ACTNOWID)
 OR (poreq.COOApp = $ACTNOWID)
 OR (poreq.MDApp = $ACTNOWID)
 )
...
//fetch tha data from the database
	 if ($NoRowLI > 0) 
   {
    ...
    if ($Approved == 0) 
            ...
            {
              $ApprovedN = "Pending with C&P for PO Creation";
            }
            else if ($Approved == 12)
            {
    ...
    $MView = '<a title="View full Request" href="mview?rNo='.$RequestID.'"><i class="fa fa-eye"></i></a>';
if($Approved == 6)
{
  $Record .='
