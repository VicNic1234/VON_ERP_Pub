...
$ReqInfo = mysql_query("SELECT * FROM poreq LEFT JOIN department ON poreq.Deparment = department.id WHERE poreq.RequestID ='$sReqID'");
...
*/
   ...
   LEFT JOIN department ON poreq.Deparment = department.id 
    LEFT JOIN divisions ON department.DivID = divisions.divid  
    WHERE
    (divisions.GM='".$myGMID."' AND poreq.Approved=4 AND poreq.isActive=1 )
    OR (poreq.Approved=6 AND poreq.isActive=1)
    OR (poreq.Approved=3 AND poreq.isActive=1 AND divisions.DH='".$myGMID."' )
    ");
    
    /* $resultRFQ1 = mysql_query("SELECT * FROM poreq");*/
...
if ($NoRowRFQ1 > 0) 
            {
             ...
             $ApprovedM =  getStatus($Approved);
                ...
                if($Approved == 4 OR $Approved == 3 OR $Approved == 6)  
                { 
...
function printDiv(divName) {
            ...
            <div class="col-md-12">
              <div class="box">
                <div class="box-header with-border">
                  <h3 class="box-title">GM <strong>[<?php echo $DivisonName; ?>]</strong> Pending Approval</h3>
   ...
   <div class="form-group has-feedback" style="width:40%; display: inline-block; margin:12px; ">
    ...
    <div class="form-group has-feedback">
      <!--<div>
        <?php if($sReqID != "" AND ($ApprovedChk == 3) || ($ApprovedChk == 4) || ($ApprovedChk == 6)) {  ?>
       <button class="btn btn-success pull-right" onclick="sendTOOfficierDD('GM')" type="button"> Send to Officer of Due Dilligence | <i class="fa fa-send"></i></button>
         <button class="btn btn-warning pull-left" onclick="sendTOBHOD('GM')" type="button"> Send to back of Head of Division | <i class="fa fa-send"></i></button>
     <?php } ?>
      </div>-->
       <div class="form-group has-feedback">
      <div>
        <?php if($sReqID != "" AND ($ApprovedChk == 4 || $ApprovedChk == 3) || ($ApprovedChk == 6)) {  ?>
       <button class="btn btn-success pull-right" onclick="sendTOOfficierDD('GM')" type="button"> Send to Officer of Due Dilligence | <i class="fa fa-send"></i></button>
        
     <?php } ?>
     
      <?php if($sReqID != "" AND ($ApprovedChk == 4) AND $RDH == $myGMID ) {  ?>
         <button class="btn btn-warning pull-left" onclick="sendBTOHODiv('GM')" type="button"> Send to back of Head of Division | <i class="fa fa-send"></i></button>
     <?php } else
     { ?>
     
      <?php if($sReqID != "" AND ($ApprovedChk == 3)) {  ?>
         <button class="btn btn-info pull-left" onclick="sendBTOHOD('GM')" type="button"> Send to back of Head of Department | <i class="fa fa-send"></i></button>
     <?php } else { ?>
...
function ReadLineItem(elem)
    {
     ...
     if (hhh != "")
     {     
    window.location.href ="gmcsppor?pdfid=" + hhh;
    //window.alert("JKJ");
     }
    ...
    <script type="text/javascript">
       function sendTOBHOD()
      {
        ...
        //var OptType =   loadAccType(acctclass); //'<?php echo $OptType ?>';
          ...
          var size='standart';
                  var content = '<form role="form" action="sendTOBDivHead" method="POST" ><div class="form-group">' +
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>PDF CODE: </label><input type="text" class="form-control"  name="ReqCode" value="'+ sReqID +'"  readonly ></div>' +
      ...
      function sendTOOfficierDD()
      {
        ...
        //var OptType =   loadAccType(acctclass); //'<?php echo $OptType ?>';
          ...
          var size='standart';
                  var content = '<form role="form" action="sendTOOfficierDD" method="POST" >'+
                  '<div class="form-group">' +
                   '<input type="hidden" value="" id="accID" name="accID" />'+
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>PDF CODE: </label><input type="text" class="form-control"  name="ReqCode" value="'+ sReqID +'"  readonly ></div>' +
                    '<input type="hidden" name="actor" value="CSGM of Divison">'+
