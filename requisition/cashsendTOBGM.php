...
while ($rowPOREQ = mysql_fetch_array($sqlPOREQ)) {
      $REQUID = $GM = $rowPOREQ['MgrApp']; $DeptHeadApp = $rowPOREQ['DeptHeadApp']; $DivHeadApp = $rowPOREQ['DivHeadApp']; $DDOfficerAppDate= $rowPOREQ['DDOfficerAppDate']; $DDOfficerAppComment = $rowPOREQ['DDOfficerAppComment'];
     }

$RoleM = "GM of Division"; 
//Confirm there is GM else send to Head of Dept
if($GM == 0)
{
  $GM =  $DivHeadApp; 
  $RoleM = "Head of Division";
}
...
if($RoleM == "GM of Division")
{
     mysql_query("UPDATE cashreq SET Approved='4', LastActor='Internal Control', Status='Sent Back', DDOfficerApp='".$uid."', ApprovedBy='".$staffname."', DDOfficerAppDate='".$TodayD."',  DDOfficerAppComment='".$ReqMSG."' WHERE RequestID = '$ReqCode'");
}
else if($RoleM == "Head of Division")
{
     mysql_query("UPDATE cashreq SET Approved='3', LastActor='Internal Control', Status='Sent Back', DDOfficerApp='".$uid."', ApprovedBy='".$staffname."', DDOfficerAppDate='".$TodayD."',  DDOfficerAppComment='".$ReqMSG."' WHERE RequestID = '$ReqCode'");
}
else if($RoleM == "Head of Dept")
{
     mysql_query("UPDATE cashreq SET Approved='2', LastActor='Internal Control', Status='Sent Back', DDOfficerApp='".$uid."', ApprovedBy='".$staffname."', DDOfficerAppDate='".$TodayD."',  DDOfficerAppComment='".$ReqMSG."' WHERE RequestID = '$ReqCode'");
}
//echo mysql_error($dbhandle);
//exit;
//now we have to inform the HOD
require_approval_action_notification ($REQEmail, $REQFullname, $ReqCode, $RoleM, $ReqMSG); 
...
function require_approval_action_notification ($email, $fullnme, $pdfid, $role, $msg)
{
        ...
        //$msgLink = 'https://www.elshcon.space/requisition/cashmgppor?pdfid='.$pdfid;
         $msgLink = 'https://www.elshcon.space';
