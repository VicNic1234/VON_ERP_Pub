...
////CHK GM CS
 $sqlGMCS=mysql_query("SELECT * FROM users WHERE COO='1'");
while ($rowGMSC = mysql_fetch_array($sqlGMCS)) {
      $REQUID = $GMDDID = $rowGMSC['uid'];
     }    
////////////////////////////////////////
...
if($isHOD < 1) { 
	$_SESSION['ErrMsg'] = "COO is not configured. Thanks";
...
//Update the rpo table
  // mysql_query("UPDATE cashreq SET Approved='8', LastActor='MD\'s Office' , Status='Sent Back', MDApp='".$uid."', ApprovedBy='".$staffname."', MDAppDate='".$TodayD."',  MDComment='".$ReqMSG."' WHERE RequestID = '$ReqCode'");
   mysql_query("UPDATE cashreq SET Approved='18', LastActor='MD\'s Office' , Status='Sent Back', MDApp='".$uid."', ApprovedBy='".$staffname."', MDAppDate='".$TodayD."',  MDComment='".$ReqMSG."' WHERE RequestID = '$ReqCode'");

//now we have to inform the HOD
require_approval_action_notification ($REQEmail, $REQFullname, $ReqCode, "COO", $ReqMSG); 
...
function require_approval_action_notification ($email, $fullnme, $pdfid, $role, $msg)
{
        ...
        $msgLink = 'https://www.elshcon.space/requisition/cashmdcoo?pdfid='.$pdfid;
...
///////////////////////////////////////
$_SESSION['ErrMsgB'] = "Request have been sent back to ".$REQFullname." as COO. Thanks";
header('Location: cashmdppor?pdfid='.$ReqCode);
