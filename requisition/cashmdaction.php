...
$sqlPOREQ=mysql_query("SELECT * FROM cashreq WHERE RequestID='$ReqCode'");
while ($rowPOREQ = mysql_fetch_array($sqlPOREQ)) {
       $MDAppDate = $rowPOREQ['MDAppDate'];  $MDComment = $rowPOREQ['MDComment'];
     }
...
//FOR APPROV
   mysql_query("UPDATE cashreq SET Approved='".$Apn."', LastActor='MD\'s Office', Status='".$ReqMSG."', MDApp='".$uid."', ApprovedAmount='".$AAMT."', ApprovedBy='".$staffname."', MDAppDate='".$TodayD."',  MDComment='".$Mgs."' WHERE RequestID = '$ReqCode'");
...
function require_approval_action_notification ($email, $fullnme, $pdfid, $role, $msg)
{
       ...
       // $GLOBALS['mail']->AddCC("henry.oshikoya@elshcon.com", "HENRY AYODELE");
        ...
        $msgBody = 'Kindly click on link below to view request on Elshcon ERP which have been Treated By MD\'s Office,<br/><br/> with message : <br/><br/><em>'.$msg.'</em>';
