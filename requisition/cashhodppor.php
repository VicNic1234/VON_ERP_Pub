...
///////////////////
//GET REQ ID BY YOU /////
$resultRFQ1 = mysql_query("SELECT DISTINCT RequestID FROM cashreq WHERE Approved>10");
...
/////////////////////////////// 
$resultStaff = mysql_query("SELECT * FROM department"); //where id='".$DeptIDn."'
...
$resultRFQ1 = mysql_query("SELECT DISTINCT RequestID, Approved , Deparment FROM cashreq WHERE (Deparment= '$DeptIDn' OR Deparment= '$DPTID') AND Approved=2 AND isActive=1");
$NoRowRFQ1 = mysql_num_rows($resultRFQ1);
...
function ReadLineItem(elem)
    {
     ...
     if (hhh != "")
     {     
    window.location.href ="cashhodppor?pdfid=" + hhh;
    //window.alert("JKJ");
     }
     ...
     <script type="text/javascript">
      function sendBTOUser()
      {
        ...
        //var OptType =   loadAccType(acctclass); //'<?php echo $OptType ?>';
          ...
          var size='standart';
                  var content = '<form role="form" action="cashsendTOBUserHDept" method="POST" ><div class="form-group">' +
      ...
      function sendBTOSupervisor()
      {
        ...
        //var OptType =   loadAccType(acctclass); //'<?php echo $OptType ?>';
          ...
          var size='standart';
                  var content = '<form role="form" action="cashsendTOBSupervisor" method="POST" ><div class="form-group">' +
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Cash Request Code: </label><input type="text" class="form-control"  name="ReqCode" value="'+ sReqID +'"  readonly ></div>' +
          ...
          var size='standart';
                  ...
                  '<div class="form-group">' +
                   '<input type="hidden" value="" id="accID" name="accID" />'+
                     '<input type="hidden"  value="HOD" id="Hrole" name="role" />'+
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Cash Request Code: </label><input type="text" class="form-control"  name="ReqCode" value="'+ sReqID +'"  readonly ></div>' +
    ...
    <script type="text/javascript">
      function sendTODivHead()
      {
        ...
        var DivisonName = '<?php echo $DivisonName ?>';
          ...
          var size='standart';
                  var content = '<form role="form" action="cashsendTODivHead" method="POST" >'+
                  '<div class="form-group">' +
                   '<input type="hidden" value="" id="accID" name="accID" />'+
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Cash Request Code: </label><input type="text" class="form-control"  name="ReqCode" value="'+ sReqID +'"  readonly ></div>' +
