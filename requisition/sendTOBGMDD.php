...
while ($rowPOREQ = mysql_fetch_array($sqlPOREQ)) {
       $GMDD = $rowPOREQ['DDApp'];
       $MDAppDate = $rowPOREQ['MDAppDate'];
       $MDComment = $rowPOREQ['MDComment'];
     }
...
if($MDComment != "")
     {
        $ReqMSG = '<div class="rcorners1">'.$MDComment.'  | <b>'.$MDAppDate.'</b></div> <br/> '. $ReqMSG;
     }
//Update the rpo table
   mysql_query("UPDATE poreq SET Approved='8', LastActor='MD\' Office' , Status='Sent Back', MDApp='".$uid."', ApprovedBy='".$staffname."', MDAppDate='".$TodayD."',  MDComment='".$ReqMSG."' WHERE RequestID = '$ReqCode'");
