...
while ($rowPOREQ = mysql_fetch_array($sqlPOREQ)) {
       $DeparmentID = $rowPOREQ['Deparment'];  $DeptHeadApp = $rowPOREQ['DeptHeadApp'];  $DivHeadAppDate = $rowPOREQ['DivHeadAppDate']; $DivHeadAppComment = $rowPOREQ['DivHeadAppComment'];
     }
...
if($REQUID == 0) { $REQUID = $DeptHeadApp; }
//echo $REQUID ."FGF"; exit;
////////////////////////////////////////
$sql_res=mysql_query("SELECT * FROM users WHERE uid='$REQUID'");
...
//Update the rpo table
   mysql_query("UPDATE cashreq SET Approved='2', LastActor='Head of Division', Status='Sent Back', DivHeadApp='".$SNDUID."', ApprovedBy='".$staffname."', DivHeadAppDate='".$TodayD."',  DivHeadAppComment='".$ReqMSG."' WHERE RequestID = '$ReqCode'");
