...
$resultRFQ1 = mysql_query("SELECT DISTINCT RequestID, Approved, Deparment FROM cashreq 
   WHERE Approved=18 AND isActive=1");
...
function printDiv(divName) {
            ...
            <div class="col-md-12">
              <div class="box">
                <div class="box-header with-border">
                 <h3 class="box-title"> <strong>COO's Office</strong> Cash Request Pending Approval</h3>
   ...
   <div class="form-group has-feedback" style="width:90%; display: inline-block; margin:12px; ">
    ...
    <div class="form-group has-feedback">
      <div>
         <?php if($sReqID != "" AND $ApprovedChk == 18) {  ?>
          <?php { ?>
            <button class="btn btn-success pull-right" onclick="sendTOMD()" type="button"> Send to MD | <i class="fa fa-send"></i></button>
       
     <?php } ?>
         <button class="btn btn-warning pull-left" onclick="sendTOBGMDD()" type="button"> Send back to Internal Control | <i class="fa fa-send"></i></button>
     <?php } ?>
      </div>
    </div>
    <script type="text/javascript">
      function MDAction(elem)
    {
     ...
     {     
         ...
         if(ActionOfMD != "MDKeepInView" && ActionOfMD != "MDCancel")
         {
             if(AAMT > 0)
             {
                window.location.href ="cashcooaction?pdfid=" + ReQID + "&ActionOfMD="+ActionOfMD+"&Mgs="+MsgMD+"&AAMT="+AAMT;
             }
              else
             {
     ...
     function sendTOMD()
      {
        ...
        //var OptType =   loadAccType(acctclass); //'<?php echo $OptType ?>';
          ...
          var size='standart';
                  var content = '<form role="form" action="cashsendTOMD" method="POST" ><div class="form-group">' +
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Cash Request Code: </label><input type="text" class="form-control"  name="ReqCode" value="'+ sReqID +'"  readonly ></div>' +
       ...
       // var DivisonName = '<?php echo $DivisonName ?>';
          ...
          var size='standart';
                  ...
                  '<div class="form-group">' +
                   '<input type="hidden" value="" id="accID" name="accID" />'+
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Cash Request Code: </label><input type="text" class="form-control"  name="ReqCode" value="'+ sReqID +'"  readonly ></div>' +
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Internal Control: </label><input type="text" class="form-control"   value="'+ GMDDNme +'"  readonly ></div>' +
                  ...
                  ;
                  var title = 'Send Back To Internal Control';
                  var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>';
...
function ReadLineItem(elem)
    {
     ...
     if (hhh != "")
     {     
    window.location.href ="cashmdcoo?pdfid=" + hhh;
    //window.alert("JKJ");
     }
