...
$sqlDeptDiv=mysql_query("SELECT * FROM department WHERE id='$Deparment'");
while ($rowDept = mysql_fetch_array($sqlDeptDiv)) {
       $DivID = $rowDept['DivID'];
     }
     
$sqlDiv=mysql_query("SELECT * FROM divisions WHERE divid='$DivID'");
while ($rowDiv = mysql_fetch_array($sqlDiv)) {
       $HeadOfDivID = $rowDiv['DH'];
     }
