...
/////////////////////////////// 
$resultStaff = mysql_query("SELECT * FROM users where uid='".$SupervisorID."'"); 
...
$resultRFQ1 = mysql_query("SELECT DISTINCT RequestID, Approved , Deparment FROM cashreq WHERE Deparment= '$DeptIDn' AND isActive=1");
$NoRowRFQ1 = mysql_num_rows($resultRFQ1);
...
else 
{
 //$result = mysql_query("SELECT * FROM poreq WHERE Deparment= '".$DeptIDn."'");
  //echo "Content have expired. Thanks";
  //exit;
}
...
if ($NoRow > 0) 
{
  ...
  while ($row = mysql_fetch_array($result)) {
     ...
     $ReqID = $row['RequestID'];
     $rPDF = '<a href="mview?rNo='.$row['relatedPDF'].'" target="_blank">'. $row['relatedPDF']. '</a>';
            ...
            <div class="col-md-12">
              <div class="box">
                <div class="box-header with-border">
                  <h3 class="box-title">Supervisor <strong>[<?php echo $Department; ?>]</strong> Pending Approval</h3>
   ...
   <div class="form-group has-feedback" style="width:90%; display: inline-block; margin:12px; ">
    ...
    <div class="form-group has-feedback">
      <div>
        ...
        <?php if($sReqID != "" AND $ApprovedCHK == 1) {  ?>
       <button class="btn btn-success pull-right" onclick="sendTODeptHead()" type="button"> Send to Head of Department | <i class="fa fa-send"></i></button>
 ...
 <script type="text/javascript">
      function sendTODeptHead()
      {
        ...
        //var OptType =   loadAccType(acctclass); //'<?php echo $OptType ?>';
          ...
          var size='standart';
                  var content = '<form role="form" action="cashsendTOHODS" method="POST" ><div class="form-group">' +
     ...
     <script type="text/javascript">
      function sendBTOUser()
      {
        ...
        //var OptType =   loadAccType(acctclass); //'<?php echo $OptType ?>';
          ...
          var size='standart';
                  var content = '<form role="form" action="cashsendTOBUser" method="POST" ><div class="form-group">' +
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Cash Request Code: </label><input type="text" class="form-control"  name="ReqCode" value="'+ sReqID +'"  readonly ></div>' +
...
function ReadLineItem(elem)
    {
     ...
     if (hhh != "")
     {     
    window.location.href ="cashsupervisorppor?pdfid=" + hhh;
    //window.alert("JKJ");
     }
        ...
        //var OptType =   loadAccType(acctclass); //'<?php echo $OptType ?>';
          ...
          var size='standart';
                  ...
                  '<div class="form-group">' +
                   ...
                   '</form>' +
                   '<form id="sendBKForm" role="form" action="sendTOReq" method="POST" >'+
                   ...
                   '<button type="button" onclick="setMSG()" class="btn btn-warning pull-left">Send Back To Requester </button><br/>'+
                   '</form>';
                  var title = 'Act As Head of Department';
                  var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>';
