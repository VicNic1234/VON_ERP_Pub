...
////////////////////////////////////
$resultStaff = mysql_query("SELECT * FROM department where hod='".$HODID."'"); 
...
$resultDiv = mysql_query("SELECT * FROM divisions WHERE divid =3");
...
$resultRFQ1 = mysql_query("SELECT DISTINCT RequestID, Approved, Deparment FROM poreq 
   WHERE (Approved=5  OR (Deparment= '$DeptIDn' AND Approved=2 )) AND isActive=1"); //OR Deparment= '$DeptIDn'
...
//////////////////////////////////////////////////////////////////////////////////////////
$resultDept = mysql_query("SELECT * FROM department WHERE id ='$ReDept'");
 while ($row = mysql_fetch_array($resultDept)) {
     //$reqid = $row['reqid'];
       $Department = $row['DeptmentName'];
       $RepDivID = $row['DivID'];
     }
  $resultDiv = mysql_query("SELECT * FROM divisions WHERE divid ='$RepDivID'");
 while ($row = mysql_fetch_array($resultDiv)) {
       $ReDivisonName = $row['DivisonName'];
     }
...
function printDiv(divName) {
            ...
            <div class="col-md-12">
              <div class="box">
                <div class="box-header with-border">
                  <h3 class="box-title">Officer <strong>[Contracts and Procument]</strong> Pending Approval</h3>
   ...
   <div class="form-group has-feedback" style="width:90%; display: inline-block; margin:12px; ">
    ...
    <div class="form-group has-feedback">
      <div>
        <?php if(($sReqID != "" AND $ApprovedChk == 5) || ($ApprovedChk == 2   )) {  ?>
          <?php { ?>
       <button class="btn btn-success pull-right" onclick="sendTOGMCS()" type="button"> Send to GM Coporate Services | <i class="fa fa-send"></i></button>
     <?php } ?>
     <?php if(($ApprovedChk == 2) ) { ?>
    ...
    <?php } else if ($RepDivID != 1) { ?>
         <button class="btn btn-warning pull-left" onclick="sendTOBGM()" type="button"> Send back to GM <?php echo $ReDivisonName; ?>  | <i class="fa fa-send"></i></button>
     <?php } 
     else { ?>
        
        <button class="btn btn-warning pull-left" onclick="sendTOBHOD()" type="button"> Send back to Head of Department | <i class="fa fa-send"></i></button>
    <?php }
   } ?>
      </div>
    </div>
          ...
          var size='standart';
                  var content = '<form role="form" action="sendTOBHOD" method="POST" ><div class="form-group">' +
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>PDF CODE: </label><input type="text" class="form-control"  name="ReqCode" value="'+ sReqID +'"  readonly ></div>' +
                    ...
                    '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Message to HOD: </label><input type="text" class="form-control" id="eaccCode" name="ReqMSG" placeholder=""  value="Kindly treat. Thanks" required ></div>' +
        ...
        var sReqID = '<?php echo $sReqID ?>';
          ...
          var size='standart';
                  ...
                  '<div class="form-group">' +
                   '<input type="hidden" value="HODCP" id="accID" name="accID" />'+
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>PDF CODE: </label><input type="text" class="form-control"  name="ReqCode" value="'+ sReqID +'"  readonly ></div>' +
                  ...
                  var title = 'Send To GM of Coporate Services';
                  var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>';
        ...
        //var OptType =   loadAccType(acctclass); //'<?php echo $OptType ?>';
          ...
          var size='standart';
                  var content = '<form role="form" action="sendTOBUser" method="POST" ><div class="form-group">' +
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>PDF CODE: </label><input type="text" class="form-control"  name="ReqCode" value="'+ sReqID +'"  readonly ></div>' +
                    '<input type="hidden" name="actor" value="HeadOfDept" />'+
     ...
     <script type="text/javascript">
       function sendTOBGM()
      {
        ...
        var DivisonName = '<?php echo $DivisonName ?>';
          ...
          var size='standart';
                  var content = '<form role="form" action="sendTOBGM" method="POST" >'+
                  '<div class="form-group">' +
    ...
    <script type="text/javascript">
       function sendTOBGMCS()
      {
        ...
        var DivisonName = '<?php echo $DivisonName ?>';
          ...
          var size='standart';
                  var content = '<form role="form" action="sendTOBGMCS" method="POST" >'+
                  '<div class="form-group">' +
                   '<input type="hidden" value="" id="accID" name="accID" />'+
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>PDF CODE: </label><input type="text" class="form-control"  name="ReqCode" value="'+ sReqID +'"  readonly ></div>' +
                  ...
                  ;
                  var title = 'Send Back To GM Corporate Services';
                  var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>';
...
function ReadLineItem(elem)
    {
     ...
     if (hhh != "")
     {     
    window.location.href ="cnpppor?pdfid=" + hhh;
    //window.alert("JKJ");
     }
