...
if($SNDUID <1) { header('Location: ../users/logout');  exit; }
...
$sqlPOREQ=mysql_query("SELECT * FROM poreq WHERE RequestID='$ReqCode'");
        while ($rowPOREQ = mysql_fetch_array($sqlPOREQ)) {
              ...
              if($DDAppComment != "")
             {
                $hodMSG = '<div class="rcorners1">'.$DDAppComment.'  | <b>'.$DDAppDate.'</b></div> <br/> '. $hodMSG;
             }

//Update the rpo table
    mysql_query("UPDATE poreq SET Approved='9', LastActor='Mgr Due Dilligence', Status='Approved', MDApp='".$MDID."', DDApp='".$uid."', ApprovedBy='".$staffname."', DDAppDate='".$TodayD."',  DDAppComment='".$hodMSG."' WHERE RequestID = '$ReqCode'");
...
function require_approval_action_notification ($email, $fullnme, $pdfid, $role, $msg)
{
        ...
        $msgLink = 'https://www.elshcon.space/requisition/mdppor?pdfid='.$pdfid;
		include('../notify.php');
