...
////////////////////////////////////
 $resultDIV = mysql_query("SELECT * FROM divisions LEFT JOIN users ON divisions.GM = users.uid WHERE divisions.GM ='$myGMID'");
 //Array of all Divisions where He is GM
 $ApprovDIVs = array();
 while ($row = mysql_fetch_array($resultDIV)) {
        $gmDIVID = $row['divid'];
       $DivisonName = $row['DivisionName'];
       $GM = $row['GM'];
        array_push($ApprovDIVs, $gmDIVID);
       $HeadOfDiv = $row['Firstname']. " " .$row['Surname'];
     }

  ////////////////////////////////////////////////////////////// 
  $sReqID = mysql_real_escape_string(trim(strip_tags($_GET['pdfid'])));
$ReqInfo = mysql_query("SELECT * FROM cashreq LEFT JOIN department ON cashreq.Deparment = department.id WHERE cashreq.RequestID ='$sReqID'");
        while ($row = mysql_fetch_array($ReqInfo)) 
        {
              $RDepartment = $row['DeptmentName'];
              $RDivID = $row['DivID'];
        }
        
$resultRDiv = mysql_query("SELECT * FROM divisions WHERE divid ='$RDivID'");
 while ($row = mysql_fetch_array($resultRDiv)) {
...
/*
 LEFT JOIN department ON cashreq.Deparment = department.id 
    ...
    , department.DivID As DivID,
divisions.DivisionName As DivNme
*/
$resultRFQ1 = mysql_query("SELECT DISTINCT RequestID, Approved, Deparment FROM cashreq 
  LEFT JOIN department ON cashreq.Deparment = department.id 
    LEFT JOIN divisions ON department.DivID = divisions.divid 
    WHERE (divisions.DH='".$myGMID."'  AND cashreq.Approved=3 AND cashreq.isActive=1) OR (divisions.GM='".$myGMID."'  AND cashreq.Approved=4 AND cashreq.isActive=1) OR (cashreq.Approved=6 AND cashreq.isActive=1)");
    $NoRowRFQ1 = mysql_num_rows($resultRFQ1); 
...
if ($NoRow > 0) 
{
  ...
  while ($row = mysql_fetch_array($result)) {
     ...
     else 
     {
      ...
      $Detele = ''; $Edit = '';
      if($ApprovedChk == 3 || $ApprovedChk == 4)
      {
 ...
 while ($row = mysql_fetch_array($sresultDept)) {
     ...
     //Let's check the Head of Div
      $sresultDiv = mysql_query("SELECT * FROM divisions WHERE divid ='$DivID'"); 
 while ($row = mysql_fetch_array($sresultDiv)) {
     //$reqid = $row['reqid'];
       $CHKDIV = $row['DH'];
    }
...
function printDiv(divName) {
            ...
            <div class="col-md-12">
              <div class="box">
                <div class="box-header with-border">
                  <h3 class="box-title">GM of Division <strong>[<?php echo $DivisionName; ?>]</strong> Pending Approval</h3>
   ...
   <div class="form-group has-feedback" style="width:90%; display: inline-block; margin:12px; ">
    ...
    <div class="form-group has-feedback">
      ...
      <div>
        <?php if($sReqID != "" AND ($ApprovedChk == 4 || $ApprovedChk == 3) ) {  ?>
         
       <button class="btn btn-success pull-right" onclick="sendTOOfficierDD()" type="button"> Send to Officer of Due Dilligence | <i class="fa fa-send"></i></button>
       <?php } ?>
       
       <?php if($sReqID != "" AND ($ApprovedChk == 4) AND ($RDH != $myGMID ))   {  ?>
       <button class="btn btn-primary pull-left" onclick="sendBTOHODiv()" type="button"> Send back to Head of Division | <i class="fa fa-send"></i></button>
       <!--<button class="btn btn-warning pull-left" onclick="sendBTOHODiv()" type="button"> Send back to Head of Division | <i class="fa fa-send"></i></button>-->
       <?php } else { ?>
        <button class="btn btn-primary pull-left" onclick="sendBTOHOD()" type="button"> Send back to Head of Department | <i class="fa fa-send"></i></button>
       <?php } ?>
       <!--
   ...
   <script type="text/javascript">
        ...
        var sReqID = '<?php echo $sReqID ?>';
          ...
          var size='standart';
                  var content = '<form role="form" action="cashsendTOBHOD" method="POST" ><div class="form-group">' +
                   ...
                   '<input type="hidden" name="actor" value="GMCS" />'+
      ...
      function sendBTOHODiv()
      {
      
        var sReqID = '<?php echo $sReqID ?>';
        
          var size='standart';
                  var content = '<form role="form" action="cashsendTOBHODiv" method="POST" ><div class="form-group">' +
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Cash Request Code: </label><input type="text" class="form-control"  name="ReqCode" value="'+ sReqID +'"  readonly ></div>' +
                   
                    '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Message to Head of Department: </label><input type="text" class="form-control" id="eaccCode" name="ReqMSG" placeholder=""  value="Kindly update and submit again. Thanks" required ></div>' +

                   '<button type="submit" class="btn btn-info pull-right">Send</button><br/></form>';
                  var title = 'Send Back To Head of Division';
                  var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>';
      ...
      function sendTOOfficierDD()
      {
        ...
        var sReqID = '<?php echo $sReqID ?>';
          ...
          var size='standart';
                  ...
                  var content = '<form role="form" action="cashsendTOGMDD" method="POST" >'+
...
function ReadLineItem(elem)
    {
     ...
     if (hhh != "")
     {     
    window.location.href ="cashgmppor?pdfid=" + hhh;
    //window.alert("JKJ");
     }
                   ...
                   '<div class="form-group" style="width:100%; display: inline-block;"><label>Description: </label><textarea class="form-control" id="EditDes" name="EditDes" placeholder="Description"></textarea></div>' +
                    ...
                    /*
                     '<div class="form-group has-feedback col-md-3"><label>Overwrite Attachment:</label> <input type="file" class="form-control" id="EditAttach" name="filed"  /></div>' + */
                      '<div class="form-group col-md-6"><label>Add Document (Optional) :</label><br/>'+
        '<span onclick="adddoc()" class="btn btn-info form-control" style="font-size: 17px; color:green" title="Click to Attach File"> Click to Attach File <i class="fa fa-plus"></i> <i class="fa fa-file"></i></span></div>'+

         '<div class="form-group col-md-12" id="bulidlink"></div>' +
