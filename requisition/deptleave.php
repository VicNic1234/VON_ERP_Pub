...
if ($NoRow > 0) 
{
	...
	while ($row = mysql_fetch_array($resultLeave)) {
     ...
     $Approved = $row['Status'];
      ...
      {
         $Approved = '<a href="BackToRequester?id='.$reqid.'"><button title="By Clicking, you send back to Requester" class="btn btn-warning"><i class="fa fa-undo"></i> Send To Requester</button></a> &nbsp; <a href="SendToDivH?id='.$reqid.'"><button title="By Clicking, you send to Head of Division" class="btn btn-success"><i class="fa fa-send"></i> Send to Head of Division</button></a>';
      }
      else if ($Approved == 2)
      {
