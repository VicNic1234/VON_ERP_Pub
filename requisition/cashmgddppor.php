...
////////////////////////////////////
$resultStaff = mysql_query("SELECT * FROM divisions where divid='3' AND GM='$HODID'"); 
...
$resultRFQ1 = mysql_query("SELECT DISTINCT RequestID, Approved, Deparment FROM cashreq 
   WHERE Approved=8 AND isActive=1");
...
function printDiv(divName) {
            ...
            <div class="col-md-12">
              <div class="box">
                <div class="box-header with-border">
                  <h3 class="box-title">GM <strong>[Internal Audit/Due Diligence]</strong> Pending Approval</h3>
   ...
   <div class="form-group has-feedback" style="width:90%; display: inline-block; margin:12px; ">
    ...
    <div class="form-group has-feedback">
      <div>
        <?php if($sReqID != "" AND $ApprovedChk == 8) {  ?>
          <?php { ?>
       <button class="btn btn-success pull-right" onclick="sendTOCOO()" type="button"> Send to COO | <i class="fa fa-send"></i></button>
     <?php } ?>
        <!-- <button class="btn btn-warning pull-left" onclick="sendTOBDDOfficer()" type="button"> Send back to GM | <i class="fa fa-send"></i></button> -->
         <button class="btn btn-warning pull-left" onclick="sendTOBGM()" type="button"> Send back to GM | <i class="fa fa-send"></i></button>
     <?php } ?>
      </div>
    </div>
   <script type="text/javascript">
      function sendTOCOO()
      {
        ...
        //var OptType =   loadAccType(acctclass); //'<?php echo $OptType ?>';
          ...
          var size='standart';
                  var content = '<form role="form" action="cashsendTOCOO" method="POST" ><div class="form-group">' +
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Cash Request Code: </label><input type="text" class="form-control"  name="ReqCode" value="'+ sReqID +'"  readonly ></div>' +
                    ...
                    '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Message to COO: </label><input type="text" class="form-control" id="eaccCode" name="ReqMSG" placeholder=""  value="Kindly treat. Thanks" required ></div>' +
    ...
    <script type="text/javascript">
       ...
       function sendTOBGM()
      {
        ...
        var sReqID = '<?php echo $sReqID ?>';
          ...
          var size='standart';
                  //var content = '<form role="form" action="cashsendTOBDDOfficer" method="POST" >'+
                  var content = '<form role="form" action="cashsendTOBGM" method="POST" >'+
                  ...
                  ;
                  var title = 'Send Back To Requester\'s GM/HDIV/HOD';
                  var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>';
...
function ReadLineItem(elem)
    {
     ...
     if (hhh != "")
     {     
    window.location.href ="cashmgddppor?pdfid=" + hhh;
    //window.alert("JKJ");
     }
