...
*/
 ...
 vendorsinvoices.IVNo AS IVNo, vendorsinvoices.Status As IVStatus FROM vendorsinvoices 
 LEFT JOIN suppliers ON vendorsinvoices.VendSource = suppliers.supid
 LEFT JOIN users ON vendorsinvoices.RaisedBy = users.uid

  WHERE vendorsinvoices.Status= 5 AND vendorsinvoices.isActive=1  ORDER BY vendorsinvoices.cid
 "); //WHERE isActive=1 ORDER BY cid //GROUP BY PONUM GROUP BY IVNo
...
//fetch tha data from the database
	 if ($NoRowLI > 0) 
   {
    ...
    $DDState = ""; $DDAState = ""; $GMCSState = ""; $MDState = "";
    ...
    if($DDAppStatus == 1) { $DDState = '<i class="fa fa-legal text-green"></i>';  }
    ...
    if($MDAppStatus == 1) { $MDState = '<i class="fa fa-legal text-green"></i>';  }
    elseif ($MDOfficeComment != "") { $MDState = '<i class="fa fa-comment text-green"></i>';  }
    else { $MDState = '<i class="fa fa-edit text-red"></i>'; }
    
    

    if($GMCSAppStatus  == 1) { $GMCSState = '<i class="fa fa-legal text-green"></i>';  }
...
if($IVSTATUS == 2)  { $invStatus = '<button class="btn btn-primary btn-sm">Pending with DD </button>'; } 
