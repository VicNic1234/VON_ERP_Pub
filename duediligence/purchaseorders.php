...
*/
 $resultLI = mysql_query("SELECT *, COUNT(*) As ItemNum, SUM(poitems.unitprice * poitems.qty) As TotalMain,
   
   SUM(CASE 
        WHEN pomiscellaneous.Impact = 'ADD' 
        THEN + pomiscellaneous.price
        WHEN pomiscellaneous.Impact = 'SUBTRACT' 
        THEN - pomiscellaneous.price
        ELSE pomiscellaneous.price
    END)
   AS MeTotal,

  purchaseorders.PONo AS PONUM FROM purchaseorders 
 LEFT JOIN suppliers ON purchaseorders.VendSource = suppliers.supid
 LEFT JOIN users ON purchaseorders.RaisedBy = users.uid
 LEFT JOIN poitems ON purchaseorders.cid = poitems.PONo
 LEFT JOIN pomiscellaneous ON purchaseorders.cid = pomiscellaneous.PONo
  WHERE purchaseorders.cid>0 GROUP BY PONUM ORDER BY purchaseorders.cid
...
//fetch tha data from the database
	 if ($NoRowLI > 0) 
   {
    ...
    if($MDOfficeOn != "") { $MDState = '<i class="fa fa-legal text-green"></i>';  }
    ...
    $ViewCon = '<a title="View full PO" href="viewpo?poid='.$cid.'"><i class="fa fa-edit"></i></a>';
    $PrintPO = '<form action="printPO" method="POST">
