...
if ($NoRowClass > 0) {
        ...
        if($ACCTID != ""){
          $resultChartMaster = mysql_query("SELECT *, postings.ChqNo As ChqNoM, postings.tncid As TNCID FROM postings
            ...
            if($FromD == "")
            {
                ...
                $RecChartMaster .= '<tr><td><b>'.$AccTClass.'</b></td><td><em><b>'.$tname.'</em></td><td>'.$acid.'</td><td><a href="viewacct?TID='.$cid.'"><i class="fa fa-eye"></i></a> '.$name.'</td><td>'.$TD.'</td><td><a href="viewacct?ACCTID='.$mid2.'"><i class="fa fa-eye text-green"></i></a> '.$mid3.' '.$CHPK.'</td><td>'.$TDescr.'</td><td>'.$ReceivedByM.'</td><td>'.number_format($DRD,2).'</td><td>'.number_format($CRD,2).'</td>
  ...
  $RecChartMasterOB .= '<tr><td><b>(Open Balance)</b></td><td><em></em></td><td>'.$acid.'</td><td>'.$name.'</td><td>-</td><td>-</td><td>--</td><td>--</td><td>'.number_format($DRDOB,2).'</td><td>'.number_format($CRDOB,2).'</td>
                    <td>'.$DelBtn.'</td></tr>';
  }
         ...
         function updateVAL(tnic)
         {
            ...
            success: function(html)
            {
                  ...
                  var content = '<div class="row" style="background:#EEE6E6; border-radius:5px;" > <div class="col-md-12"><form role="form" action="updatepost" method="post" ><div class="form-group">' +
                   ...
                   '<div class="form-group col-md-12"><label>Post Description: </label><textarea class="form-control"  name="PostDesc" id="PostDesc" required >'+GLDescription+'</textarea></div>' +
