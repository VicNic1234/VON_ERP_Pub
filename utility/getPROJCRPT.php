...
if($WkNO == 0 || $WkNO == "")
{
    ...
    while ($row = mysql_fetch_array($resultRPO)) { 
      ...
      $GrowthEngine .= '<tr><td>'.$row['BussinessUnit'].'</td><td>'.$row['Count'].'</td><td>'.$row['Currency'].' '.number_format(ceil($row['ExtendedCost'])).'</td></tr>';
      $NOPO = $NOPO + $row['Count'];
    ...
    while ($row = mysql_fetch_array($resultpCUS)) { 
      ...
      $PERCUS .= '<tr><td>'.$row['Customer'].'</td><td>'.$row['Count'].'</td><td>'.$row['Currency'].' '.number_format(ceil($row['ExtendedCost'])).'</td></tr>';
    ...
    while ($row = mysql_fetch_array($resultpOEM)) { 
      ...
      $PEROEM .= '<tr><td>'.$row['OEM'].'</td><td>'.$row['Count'].'</td><td>'.$row['Currency'].' '.number_format(ceil($row['ExtendedCost'])).'</td></tr>';
