...
//We need to get No of PO delivered and PO value
  /*$resultINPOd = mysql_query("SELECT so.Customer, so.CusONo, purchaselineitems.Status, so.SOdate As MOSDate, purchaselineitems.SOCode, COUNT(*) As ItemN , SUM(purchaselineitems.ExtendedCost) As TotalAmtQ FROM purchaselineitems INNER JOIN so ON purchaselineitems.SOCode = so.SONum INNER JOIN logistics ON purchaselineitems.LitID = logistics.lineItID WHERE (RIGHT(so.SOdate,4) = '$BusinessYr') AND logistics.FullDeli = 1  GROUP BY so.CusONo");
  $NoRowPOd = mysql_num_rows($resultINPOd);
  $AmtPOd = 0.0;

  //Total Amount Quoted
   while ($row = mysql_fetch_array($resultINPOd)) { $AmtPOd = $AmtPOd +  $row['TotalAmtQ']; 
   $ProjectSummary .= "<tr><td>".$row['Customer']."</td><td>".$row['CusONo']."</td><td>".$row['MOSDate']."</td><td>".$row['ItemN']."</td><td>".$row['TotalAmtQ']."</td></tr>";
 */
 }  

/*  $resultINPO = mysql_query("SELECT logistics.FullDeli, logistics.POID, COUNT(*),  SUM(logistics.POAmt) As TotalAmtQ FROM logistics INNER JOIN so ON purchaselineitems.SOCode = so.SONum  WHERE LEFT(logistics.FullDeliveryOn,4) = '$BusinessYr' AND logistics.FullDeli = '1' GROUP BY purchaselineitems.SOCode");
  $NoRowPO = mysql_num_rows($resultINPO);
  $AmtPO = 0.0;
...
*/
 ...
 /*  //We need to get No of PO delivered and PO value
  $resultINPOd = mysql_query("SELECT so.Customer, so.CusONo, purchaselineitems.Status, so.SOdate As MOSDate, purchaselineitems.SOCode, COUNT(*) As ItemN , SUM(purchaselineitems.ExtendedCost) As TotalAmtQ FROM purchaselineitems INNER JOIN so ON purchaselineitems.SOCode = so.SONum INNER JOIN logistics ON purchaselineitems.LitID = logistics.lineItID WHERE STR_TO_DATE(so.SOdate, '%d/%m/%Y') between '$oFirstDay' AND '$oLastDay' AND logistics.FullDeli = 1 GROUP BY so.CusONo");
  $NoRowPOd = mysql_num_rows($resultINPOd);
  $AmtPOd = 0.0;

  //Total Amount Quoted
   while ($row = mysql_fetch_array($resultINPOd)) { $AmtPOd = $AmtPOd +  $row['TotalAmtQ']; 
   $ProjectSummary .= "<tr><td>".$row['Customer']."</td><td>".$row['CusONo']."</td><td>".$row['MOSDate']."</td><td>".$row['ItemN']."</td><td>".ceil($row['TotalAmtQ'])."</td></tr>";
 */
 }
...
$TRFQOPNData = array(
 ...
 //$summary['AmtPOd'] = number_format($AmtPOd);
