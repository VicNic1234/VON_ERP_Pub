...
$CONID = $_GET['cnid'];
 $resultLI = mysql_query("SELECT * FROM docclass 
 LEFT JOIN users ON docclass.addedby = users.uid
 WHERE sdid='".$CONID."'
...
*/ 
     ...
     /*Get Supporting Docs*/
$resultSDoc = mysql_query("SELECT * FROM docs Where subid='$CONID' AND isActive=1 AND Type='Subject'");
$NoRowSDoc = mysql_num_rows($resultSDoc);
 $SN = 0;
if ($NoRowSDoc > 0) {
  while ($row = mysql_fetch_array($resultSDoc)) 
  ...
  else
  {
    ...
    if($addedby == $MYID)
    {
      $delDoc = '<a href="deldoc?id='.$lactid.'&cnid='.$sdid.'"><i class="fa fa-trash"></i></a>';
    }
    $SupDocs .= '<tr>
                      <td>'.$SN.'</td>
                      <td>'.$title.'</td>
    ...
    <?php include('leftmenu.php') ?>
      ...
      <div class="content-wrapper">
        ...
        <section class="content-header">
          <h1>
             Document Class : <?php echo $SubjTitle; ?>
    ...
    return true;
      ...
      function adddoc()
      {
        ...
        //alert(CONID);
          ...
          var size='standart';
                  var content = '<form role="form" action="addDocument" method="POST" enctype="multipart/form-data" ><div class="form-group">' +
                   '<input type="hidden" name="conID" value="'+CONID+'" required />'+
                  
                   '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Document Title: </label><input type="text" class="form-control"  name="ActTitle" required ></div>' +

                    '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Document Code: </label><input type="text" class="form-control"  name="ActCode" ></div>' +

                    '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Document Description: </label><input type="text" class="form-control"  name="ActDescr" ></div>' +
