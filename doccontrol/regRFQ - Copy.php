...
$query = "INSERT INTO rfq (RFQNum, Customer, Scope, DateRange, Source, Attachment, CompanyRefNo, PENjobCode, BuyersNme, PEAID, Comment, Ellapes, Status, MirriorRFQ, RFQDiv) 
VALUES ('$RFQNo','$RFQCus','$RFQScope','$DRange','$RFQSource','$DBfilelnk','$ReqNo','$ReqNo','$RFQCus','$PEAss','$Comment','$RFQEllapes','OPEN', '$mLIRFQ', '$RFQDiv');";
...
$regResult = mysql_query($query);
 if ($regResult)
 {
 	if ($chkmRFQNo == "on" &&  $mLIRFQ != "")
  {
	...
	$query = "INSERT INTO lineitems (RFQCode, MatSer, Description, Qty, UOM, Price, Currency, UnitCost, ExtendedCost) 
	VALUES ('".$RFQNo."','".$row{'MatSer'}."','".$row{'Description'}."','".$row{'Qty'}."','".$row{'UOM'}."','".$row{'Price'}."','".$row{'Currency'}."', '".$row{'UnitCost'}."', '".$row{'ExtendedCost'}."');";
	mysql_query($query);
	/////////////////////////////////
	$liID = mysql_insert_id();
	$query1 = "INSERT INTO polineitems (LitID, RFQCode, MatSer, Description, Qty, UOM, Price, Currency, UnitCost, ExtendedCost) 
	VALUES ('".$liID."','".$RFQNo."','".$row{'MatSer'}."','".$row{'Description'}."','".$row{'Qty'}."','".$row{'UOM'}."','".$row{'Price'}."','".$row{'Currency'}."', '".$row{'UnitCost'}."', '".$row{'ExtendedCost'}."');";
