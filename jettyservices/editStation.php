...
header('Location: stations');
 ...
 else 
 {
	...
	if($NoRowchkExist > 0)
	 { 
	   	$_SESSION['ErrMsg'] = "Location already exist";
		header('Location: stations');
		exit;
	 } 

 	$sql_res=mysql_query("UPDATE equip_stations SET station_name='$nmeClass', description='$description' WHERE cid = '$idClass'");
	mysql_query($sql_res, $dbhandle);
  
	$_SESSION['ErrMsgB'] = "Equipment Location Updated!";
	header('Location: stations');
