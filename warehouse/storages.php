...
$RecChartType = "";
$resultChartType = mysql_query("SELECT wh_storages.id As TypeID, wh_storages.name, wh_storages.class_id, wh_storages.isActive As TyIsActive, wh_stations.cid, wh_stations.station_name, wh_stations.ctype, wh_stations.isActive FROM wh_storages LEFT JOIN wh_stations
ON wh_storages.class_id=wh_stations.cid WHERE wh_storages.isActive=1 ORDER BY TypeID  ");
...
if ($NoRowChartType > 0) {
  while ($row = mysql_fetch_array($resultChartType)) {
    ...
    $RecChartType .= '<tr><td>'.$id.'</td><td>'.$name.'</td><td>'.$class_name.'</td>
    
    <td><a acctnme="'.$class_name.'" accttype="'.$name.'" acctid="'.$id.'" onclick="editAcc(this)" ><i class="fa fa-edit"></i></a></td>
    <td><a acctnme="'.$class_name.'" accttype="'.$name.'" acctid="'.$id.'" onclick="deleteAcc(this)" ><i class="fa fa-trash"></i></a></td></tr>';
    }
...
function editAcc(elem)
{
 ...
 var OptClClass = '<?php echo $OptClass ?>'
    var size='standart';
            var content = '<form role="form" action="editStorage" method="POST" ><div class="form-group">' +
             '<input type="hidden" value="'+acctid+'" id="idType" name="idType" />'+
              '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Location Name: </label><select class="form-control" id="nmeClass" name="nmeClass">'+OptClClass+'</select></div>' +
              '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Storage: </label><input type="text" class="form-control" id="accType" name="accType" value="'+accttype+'" placeholder="Storage" ></div>' +
             '<button type="submit" class="btn btn-primary pull-right">Update</button><br/></form>';
            var title = 'Edit Storage';
...
function addAcc(elem)
{
    var OptClClass = '<?php echo $OptClass ?>'
    var size='standart';
            var content = '<form role="form" action="addStorage" method="POST" ><div class="form-group">' +
              '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Location: </label><select class="form-control" id="Station" name="Station">'+OptClClass+'</select></div>' +
              '<div class="form-group" style="width:100%; display: inline-block; margin: 6px"><label>Storage Name: </label><input type="text" class="form-control" id="nStorage" name="nStorage" placeholder="New Storage" ></div>' +
              '<button type="submit" class="btn btn-primary pull-right">Add Storage</button><br/></form>';
            var title = 'New Storage';
            var footer = '<button type="button" class="btn btn-default" data-dismiss="modal">Close</button>';
           ...
           <div class="col-md-12">
              <div class="box">
                <div class="box-header with-border">
                  <h3 class="box-title">Storages</h3>
                  <div class="box-tools pull-right">
                  </div>
                </div><!-- /.box-header -->
              <div class="box">
                <div class="box-header">
                 <button class="btn btn-success pull-right" id="addAcct" onclick="addAcc(this)"> Add Warehouse Storage</button>
