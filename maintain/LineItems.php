...
//check if user exist
 ...
 if ($_GET['sRFQ'] == "")
 {
 $resultLI = mysql_query("SELECT * FROM lineitems WHERE Status='OPEN'");
//check if user exist
...
?>
  ...
  <script language="javascript">
        ...
        function open_container(LID, rfq, mst, qty, uom, lides)
        ...
        {  
            ...
            '<input type="text" class="form-control" id="LIDes" name="LIDes" placeholder="Description of Line Item" value="'+ lides +'" ></input>' +
		...
		function Delete_LineItem(LID, rfq, mst, qty, uom, lides)
		
        {  
			var title = 'Are You Sure you want to DELETE Item with ID no.: '+LID + ', in RFQ No.: '+rfq;
			
            var size='standart';
            var content = '<form role="form" method="post" action="deleteLineItem"><div class="form-group">' +
			'Line Item : <label>'+ lides  +'</label> </br>' +
			'<label>Note: After delete, you can never recover this item, thanks!  </label> </br>' +
			'<input type="hidden" name="LIID" value="'+ LID +'" ></input>' +
			'<input type="hidden" name="LIRFQ" value="'+ rfq +'" ></input>' +
	  ...
	  $RIDU = "'#".$LitID."U'";
	    ...
	    $Record .='
					 <tr>
						...
						<td>'.$RFQn.'</td>
						<td>'.$LitID.'</td>
						<td><a '.  'onclick="open_container('.$LitID.',\''.$RFQn.'\',\''.$MatSer.'\',\''.$Qty.'\',\''.$UOM.'\',\''.$Description.'\');">'. '<span class="glyphicon glyphicon-edit"></span></a></td>
						<td><a '.  'onclick="Delete_LineItem('.$LitID.',\''.$RFQn.'\',\''.$MatSer.'\',\''.$Qty.'\',\''.$UOM.'\',\''.$Description.'\');">'. '<span class="glyphicon glyphicon-trash"></span></a></td>
					 </tr>';
